...
CREATE TABLE IF NOT EXISTS wallet_metrics_by_category (
  ...
  -- Category-specific context (not in wallet_metrics_complete)
  trades_in_category UInt32 COMMENT 'Total trades in this category',
...
CREATE INDEX IF NOT EXISTS idx_category_sharpe ON wallet_metrics_by_category(category, metric_6_sharpe)
  TYPE minmax GRANULARITY 4;

CREATE INDEX IF NOT EXISTS idx_wallet_category ON wallet_metrics_by_category(wallet_address, category)
  TYPE bloom_filter(0.01) GRANULARITY 1;

-- Comments
COMMENT ON TABLE wallet_metrics_by_category IS 'All 102 metrics broken down by category - for specialist detection and category-specific Smart Money signals';
