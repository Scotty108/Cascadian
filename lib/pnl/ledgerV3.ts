...
export interface LedgerV3Result {
  wallet: string;
  buys: number;
  sells: number;
  redemptions: number;
...
interface TxEvent {
  kind: 'tx';
  time: string;
  tokens: Map<string, TxTokenStats>;
}
...
type LedgerEvent = TxEvent | CtfEvent;
...
function sortLedgerEvents(a: LedgerEvent, b: LedgerEvent): number {
...
function redeemedTokensFromPayout(payoutUsdc: number, resolutionPrice?: number | null): number {
  if (!resolutionPrice || resolutionPrice <= 0) return 0;
  return payoutUsdc / resolutionPrice;
}

export async function computeLedgerV3Pnl(wallet: string): Promise<LedgerV3Result> {
  const w = wallet.toLowerCase();

  // 1) Load deduped CLOB trades (include tx_hash so we can group fills)
  const tradesQ = `
    SELECT
      side,
  ...
  // 2) Load CTF events for the wallet (explicit splits/merges/redemptions)
  const ctfQ = `
    SELECT
      event_type,
      condition_id,
  ...
  // 4) Build condition/outcome -> token map (include redemption-only conditions)
  ...
  // 6) Group trades by tx_hash
  const txMap = new Map<string, { time: string; tokens: Map<string, TxTokenStats> }>();
  for (const t of trades) {
    const txHash = t.tx_hash || `no_tx_${t.trade_time}`;
    const entry = txMap.get(txHash) || { time: t.trade_time, tokens: new Map() };
    if (t.trade_time < entry.time) entry.time = t.trade_time;
    const stats = entry.tokens.get(t.token_id) || { buyTokens: 0, sellTokens: 0, buyUsdc: 0, sellUsdc: 0 };
    if (t.side === 'buy') {
      stats.buyTokens += t.tokens;
      stats.buyUsdc += t.usdc;
    } else {
      stats.sellTokens += t.tokens;
      stats.sellUsdc += t.usdc;
    }
    entry.tokens.set(t.token_id, stats);
    txMap.set(txHash, entry);
  }

  const txEvents: TxEvent[] = [];
  for (const [, entry] of txMap) {
    txEvents.push({ kind: 'tx', time: entry.time, tokens: entry.tokens });
  }

  const ctfLedgerEvents: CtfEvent[] = [];
  for (const e of ctfEvents) {
    const amt = Number(e.amount_or_payout || 0) / 1e6;
    if (amt <= 0) continue;
    if (e.event_type === 'PositionSplit') {
      ctfLedgerEvents.push({ kind: 'split', time: e.event_timestamp, conditionId: e.condition_id, amountUsdc: amt });
    } else if (e.event_type === 'PositionsMerge') {
      ctfLedgerEvents.push({ kind: 'merge', time: e.event_timestamp, conditionId: e.condition_id, amountUsdc: amt });
    } else {
      ctfLedgerEvents.push({ kind: 'redemption', time: e.event_timestamp, conditionId: e.condition_id, amountUsdc: amt });
    }
  }

  const events: LedgerEvent[] = [...txEvents, ...ctfLedgerEvents];
  events.sort(sortLedgerEvents);

  // 7) Ledger state
  const inventory = new Map<string, number>();
  const getInv = (tokenId: string) => inventory.get(tokenId) || 0;
  const setInv = (tokenId: string, value: number) => inventory.set(tokenId, value);
  ...
  const applySplit = (conditionId: string, amount: number, explicit: boolean) => {
    if (amount <= 0) return;
    const outcomes = outcomeMap.get(conditionId);
    if (!outcomes || outcomes.size === 0) return;
    for (const tokenId of outcomes.values()) {
      setInv(tokenId, getInv(tokenId) + amount);
    }
    splitCost += amount;
    if (explicit) explicitSplits += amount;
    else implicitSplits += amount;
  };
  ...
  const applyMerge = (conditionId: string, amount: number) => {
    ...
    if (!outcomes || outcomes.size === 0) return;
    for (const tokenId of outcomes.values()) {
      const current = getInv(tokenId);
      const use = Math.min(current, amount);
      setInv(tokenId, current - use);
    }
    merges += amount;
  };
  ...
  for (const event of events) {
    if (event.kind === 'tx') {
      // Aggregate per condition and compute deficits AFTER netting buys/sells in the tx.
      const conditionToTokens = new Map<string, Set<string>>();
      for (const tokenId of event.tokens.keys()) {
        const mapping = tokenToCondition.get(tokenId);
        if (!mapping) continue;
        const set = conditionToTokens.get(mapping.conditionId) || new Set<string>();
        set.add(tokenId);
        conditionToTokens.set(mapping.conditionId, set);
      }

      for (const [conditionId] of conditionToTokens.entries()) {
        const outcomes = outcomeMap.get(conditionId);
        if (!outcomes || outcomes.size === 0) continue;
        let maxDeficit = 0;
        for (const tokenId of outcomes.values()) {
          const stats = event.tokens.get(tokenId);
          const delta = (stats?.buyTokens || 0) - (stats?.sellTokens || 0);
          const projected = getInv(tokenId) + delta;
          const deficit = Math.max(0, -projected);
          maxDeficit = Math.max(maxDeficit, deficit);
        }
        if (maxDeficit > 0) {
          applySplit(conditionId, maxDeficit, false);
        }
      }

      // Apply net token deltas and cash flow
      for (const [tokenId, stats] of event.tokens.entries()) {
        const delta = stats.buyTokens - stats.sellTokens;
        if (delta !== 0) setInv(tokenId, getInv(tokenId) + delta);
        buys += stats.buyUsdc;
        sells += stats.sellUsdc;
      }
      continue;
    }

    if (event.kind === 'split') {
      applySplit(event.conditionId, event.amountUsdc, true);
      continue;
    }
    ...
    const available = getInv(winnerTokenId);
    if (available < redeemedTokens) {
      const deficit = redeemedTokens - available;
      applySplit(event.conditionId, deficit, false);
    }
    setInv(winnerTokenId, getInv(winnerTokenId) - redeemedTokens);
    redemptions += event.amountUsdc;
    redemptionApplied += 1;
  }

  // 8) Held value (remaining inventory)
  for (const [tokenId, amount] of inventory.entries()) {
