/**
 * V16 PnL Engine Tests - Two-Tier Router
 *
 * Goal: 15/15 accuracy against Polymarket API
 *
 * Strategy:
 * - CLOB-only wallets → V1 (fast, local)
 * - Neg Risk wallets → V7 (API-based)
 *
 * This should achieve 100% accuracy by routing Neg Risk wallets to the API.
 */
...
import { getWalletPnLV16 } from './pnlEngineV16';
...
async function runTests() {
  console.log('=== V16 PnL Engine Test Suite ===\n');
  console.log('Two-tier router: V1 for CLOB-only, V7 for Neg Risk\n');
  console.log('Name           | V16 Calc     | API (truth)  | Diff      | Source   | BundledTx | Status');
  console.log('---------------|--------------|--------------|-----------|----------|-----------|-------');
  ...
  for (const w of TEST_WALLETS) {
    try {
      const [v16Result, apiResult] = await Promise.all([getWalletPnLV16(w.wallet), getWalletPnLV7(w.wallet)]);

      const v16Total = v16Result.totalPnl;
      const apiTotal = apiResult.totalPnl;
      const diff = apiTotal !== 0 ? ((v16Total - apiTotal) / Math.abs(apiTotal)) * 100 : v16Total === 0 ? 0 : 999;

      const match = isMatch(v16Total, apiTotal);
      if (match) passed++;
      else failed++;

      if (v16Result.source === 'v1-local') localCount++;
      else apiCount++;
      ...
      results.push({
        ...
        diff,
        match,
        source: v16Result.source,
        bundledTxCount: v16Result.bundledTxCount,
      });

      const status = match ? 'PASS' : 'FAIL';
      console.log(
        `${w.name.padEnd(14)} | $${v16Total.toFixed(2).padStart(11)} | $${apiTotal.toFixed(2).padStart(11)} | ${diff.toFixed(1).padStart(8)}% | ${v16Result.source.padEnd(8)} | ${v16Result.bundledTxCount.toString().padStart(9)} | ${status}`
      );
    } catch (error) {
  ...
  console.log(`  - V1 (local): ${localCount} wallets`);
  console.log(`  - V7 (API):   ${apiCount} wallets`);

  if (failed > 0) {
    console.log('\n=== STILL FAILING ===');
    for (const f of results.filter((r) => !r.match)) {
      console.log(`${f.name}: V16=$${f.v16.toFixed(2)} API=$${f.api.toFixed(2)} (${f.diff.toFixed(1)}%)`);
    }
  }
