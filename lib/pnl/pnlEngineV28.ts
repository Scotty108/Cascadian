/**
 * PnL Engine V28 - V25 with Better Resolution Handling
 *
 * Removes split pattern detection from V26 and uses pure V25 approach
 * with corrected resolution price handling.
 *
 * @author Claude Code
 * @version 28.0.0
 * @created 2026-01-09
...
export async function getWalletPnLV28(wallet: string): Promise<PnLResultV28> {
  const w = wallet.toLowerCase();
...
if (currentUrl === scriptPath) {
  ...
  if (!wallet) {
    console.log('Usage: npx tsx lib/pnl/pnlEngineV28.ts <wallet>');
    process.exit(1);
  }
  getWalletPnLV28(wallet)
    .then(r => console.log('V28 Result:', JSON.stringify(r, null, 2)))
