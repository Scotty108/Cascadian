/**
 * Hybrid Cash Flow P&L Engine
 *
 * Combines pure cash flow with pattern-based split inference.
 *
 * For normal traders (net buyers, balanced traders):
 *   P&L = Sells + Redemptions + Merges - Buys - ExplicitSplits
 *
 * For arbitrageurs (high sell/buy ratio + token deficit):
 *   P&L = Sells + Redemptions + Merges - Buys - ExplicitSplits - InferredSplitCost
 *   Where InferredSplitCost = TokenDeficit (at $1/token)
 *
 * Arbitrageur detection:
 *   - Sell/Buy USDC ratio > 2.0
 *   - Token deficit > 0 (sold more tokens than bought)
 */

import { clickhouse } from '@/lib/clickhouse/client';

export interface HybridCashFlowResult {
  wallet: string;
  pattern: 'net_buyer' | 'balanced' | 'arbitrageur';
  buys: number;
  sells: number;
  redemptions: number;
  merges: number;
...
export async function computeHybridCashFlowPnl(wallet: string): Promise<HybridCashFlowResult> {
  ...
  const sellBuyRatio = buys > 0 ? sells / buys : (sells > 0 ? Infinity : 0);

  // 4) Determine pattern
  let pattern: 'net_buyer' | 'balanced' | 'arbitrageur';
  if (sellBuyRatio > 2.0 && tokenDeficit > 0) {
    pattern = 'arbitrageur';
  } else if (tokensBought > tokensSold) {
    pattern = 'net_buyer';
  } else {
    pattern = 'balanced';
  }

  // 5) Calculate split cost based on pattern
  let inferredSplitCost = 0;
  if (pattern === 'arbitrageur') {
    // For arbitrageurs: infer split cost from token deficit
    // Token deficit = tokens that must have come from splits
    inferredSplitCost = tokenDeficit;
  }

  const totalSplitCost = explicitSplits + inferredSplitCost;

  // 6) Calculate P&L
  const cashIn = sells + redemptions + merges;
  const cashOut = buys + totalSplitCost;
