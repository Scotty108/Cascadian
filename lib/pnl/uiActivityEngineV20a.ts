/**
 * ============================================================================
 * CASCADIAN PNL ENGINE - V20a (EXPERIMENTAL)
 * ============================================================================
 *
 * STATUS: EXPERIMENTAL - Testing PayoutRedemption inclusion
 *
 * WHAT'S DIFFERENT FROM V20:
 *   - V20:  source_type = 'CLOB' only
 *   - V20a: source_type IN ('CLOB', 'PayoutRedemption')
 *
 * EVERYTHING ELSE IS IDENTICAL:
 *   - Same formula: cash_flow + (final_tokens * resolution_price)
 *   - Same aggregation by (condition_id, outcome_index)
 *   - Same unrealized estimation at 0.5 mark price
 *
 * HYPOTHESIS:
 *   V20 misses PayoutRedemption events (USDC received when redeeming winning shares).
 *   By including them in the ledger:
 *   - token_delta from redemptions (negative, burning shares) combines with CLOB purchases
 *   - usdc_delta from redemptions (positive, receiving payout) combines with CLOB spend
 *   - Net effect should be more accurate PnL
 *
 * VALIDATION:
 *   Run scripts/pnl/compare-v20-v20a.ts to see improvement metrics.
 *
 * ============================================================================
 */
...
export interface WalletMetricsV20a {
  wallet: string;
  realized_pnl: number;
...
// -----------------------------------------------------------------------------
// V20a Quick PnL (for benchmarking)
// -----------------------------------------------------------------------------

export async function calculateV20aPnL(wallet: string): Promise<{
  total_pnl: number;
  realized_pnl: number;
  unrealized_pnl: number;
  positions: number;
...
class V20aEngine {
  async compute(wallet: string): Promise<WalletMetricsV20a> {
...
export function createV20aEngine(): V20aEngine {
  return new V20aEngine();
}
