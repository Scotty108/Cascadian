#!/usr/bin/env npx tsx
/**
 * Find Top Wallets by Asinh Score
 *
 * Approach:
 * - Get top 2000 wallets by resolved trade count (fast)
 * - Filter to those active in last 10 days
 * - Calculate asinh scores with actual exit prices
...
async function main() {
  console.log('🔍 Finding top wallets by Asinh Score (Active in 10d)...')
  console.log('')

  // Step 1: Get top 2000 wallets by resolved trade count that are active in last 10 days
  console.log('📦 Step 1: Getting top wallets by trade volume (active last 10d)...')
  const walletQuery = `
    SELECT wallet, total_trades
    FROM (
      -- Top wallets by resolved trade count
      SELECT
        lower(trader_wallet) as wallet,
    ...
    -- Filter to active in last 10 days
    WHERE wallet IN (
  ...
  const wallets = (await walletResult.json() as Array<{wallet: string, total_trades: number}>).map(w => w.wallet)
  console.log(`   ✅ Found ${wallets.length} wallets (top by trades + active 10d)`)

  // Process in batches
  console.log('')
  console.log('📊 Step 2: Calculating asinh scores...')
  const allResults: WalletResult[] = []
  const batchSize = 20 // Smaller batches for speed
  ...
  console.log(`\r   ✅ Complete! Processed ${allResults.length} wallets                    `)
  ...
  // Save to JSON
  const outputPath = './data/top-asinh-active-wallets.json'
  writeFileSync(outputPath, JSON.stringify(filtered, null, 2))
  console.log(`\n💾 Saved ${filtered.length} results to ${outputPath}`)

  // Print top 30
  console.log('')
  console.log('═'.repeat(120))
  console.log(`TOP 30 WALLETS BY ASINH SCORE (${filtered.length} total)`)
  console.log('═'.repeat(120))
  console.log('')
  console.log(
    'Rank'.padEnd(6) +
...
#${i + 1}: ${row.wallet}
  ...
  console.log('')
  console.log(`✅ Complete. ${filtered.length} wallets saved to ${outputPath}`)
}
