...
import { getWalletPnLV19 } from '../lib/pnl/pnlEngineV19';
...
async function runTests() {
  console.log('=== V19 TDD TEST SUITE ===\n');
  console.log('Name           | V19 Local    | API (truth)  | Diff      | Compl | Status');
  console.log('---------------|--------------|--------------|-----------|-------|-------');
  ...
  for (const w of TEST_WALLETS) {
    try {
      const [v19Result, apiPnl] = await Promise.all([getWalletPnLV19(w.wallet), fetchApiPnl(w.wallet)]);
      const v19Total = v19Result.totalPnl;
      const apiTotal = apiPnl || 0;
      const pctDiff = apiTotal !== 0 ? Math.abs((v19Total - apiTotal) / apiTotal) * 100 : (v19Total === 0 ? 0 : 999);
      const absDiff = Math.abs(v19Total - apiTotal);
      const match = pctDiff < 10 || absDiff < 5;
      if (match) passed++; else { failed++; failing.push(w.name + ': V19=$' + v19Total.toFixed(0) + ' API=$' + apiTotal.toFixed(0) + ' (' + pctDiff.toFixed(1) + '%)'); }
      const status = match ? 'PASS' : 'FAIL';
      const diffStr = pctDiff < 999 ? pctDiff.toFixed(1) + '%' : 'N/A';
      console.log(w.name.padEnd(14) + ' | $' + v19Total.toFixed(0).padStart(11) + ' | $' + apiTotal.toFixed(0).padStart(11) + ' | ' + diffStr.padStart(9) + ' | ' + v19Result.complementCount.toString().padStart(5) + ' | ' + status);
    } catch (error) {
