#!/usr/bin/env npx tsx
/**
 * Build Cohort PnL - Two Step Approach
 *
 * Step 1: Get active wallet list (fast)
 * Step 2: Calculate PnL only for active wallets (filtered, less memory)
 */
...
async function main() {
  ...
  console.log('BUILD COHORT PNL (Two-Step Approach)');
  console.log('='.repeat(80));

  const startTime = Date.now();

  // Step 1: Create temp table with active wallets
  console.log('\n1. Finding active wallets (>= 20 trades in last 14 days)...');

  await clickhouse.command({ query: 'DROP TABLE IF EXISTS tmp_active_wallets' });

  await clickhouse.command({
    query: `
      CREATE TABLE tmp_active_wallets
      ENGINE = MergeTree()
      ORDER BY wallet
      AS
      SELECT trader_wallet as wallet
      FROM (
        SELECT
          trader_wallet,
          count() AS trades,
  ...
  const countQ = await clickhouse.query({
    query: 'SELECT count() as cnt FROM tmp_active_wallets',
    format: 'JSONEachRow'
  });
  const activeCount = (await countQ.json() as any[])[0]?.cnt || 0;
  console.log(`   Found ${activeCount.toLocaleString()} active wallets`);

  // Step 2: Calculate PnL - process in chunks of 50k wallets
  console.log('\n2. Creating result table...');
  ...
  // Get wallet batches
  const BATCH_SIZE = 25000;  // Process 25k wallets at a time
  const numBatches = Math.ceil(activeCount / BATCH_SIZE);

  console.log(`\n3. Calculating PnL in ${numBatches} batches of ${BATCH_SIZE.toLocaleString()}...`);
    ...
    try {
      await clickhouse.command({
        query: `
          ...
          WITH
            batch_wallets AS (
              SELECT wallet FROM tmp_active_wallets LIMIT ${BATCH_SIZE} OFFSET ${offset}
            ),
            -- First filter to batch wallets (TAKER ONLY to avoid double-counting)
            -- See docs/reports/PNL_2X_BUG_ROOT_CAUSE_ANALYSIS.md for details
            filtered_events AS (
              ...
              WHERE is_deleted = 0
                ...
                AND event_id LIKE '%-t'  -- Only taker fills (fixes 2x bug)
            ),
            -- Then dedupe
            deduped_trades AS (
              SELECT
  ...
  console.log('\n4. Verifying...');
  const finalCountQ = await clickhouse.query({
    query: 'SELECT count() as cnt FROM pm_cohort_pnl_active_v1',
    format: 'JSONEachRow'
  });
  const finalCount = (await finalCountQ.json() as any[])[0]?.cnt || 0;
  console.log(`   Total wallets: ${finalCount.toLocaleString()}`);
