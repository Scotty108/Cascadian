/**
 * Quick V15 Test - NegRisk Paired Trade Consolidation
 */

import { createV15Engine } from '../../lib/pnl/uiActivityEngineV15';
...
async function main() {
  ...
  console.log('QUICK V15 TEST (NegRisk Paired Trade Consolidation)');
  console.log('='.repeat(80));

  const engine = createV15Engine();
  ...
  for (const w of TEST_WALLETS) {
    ...
    try {
      ...
      console.log(`  V15:     $${result.realized_pnl.toLocaleString()}`);
      console.log(`  Error:   ${errorPct.toFixed(1)}% ${signMatch ? '' : '[SIGN MISMATCH]'}`);
      console.log(`  Status:  ${status}`);
      console.log(`  Paired:  ${result.paired_trades_consolidated} consolidated | Trades: ${result.total_trades} | Time: ${elapsed}s`);
