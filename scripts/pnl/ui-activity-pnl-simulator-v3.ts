 ...
 * 3. IMPLICIT RESOLUTION LOSSES: If a market resolves and user holds losing tokens,
 *    treat as "sell at $0" to realize the loss
 *
 * Algorithm: Average Cost Basis + Resolution-based Loss Realization
 *
 * Usage: npx tsx scripts/pnl/ui-activity-pnl-simulator-v3.ts
 */
...
async function getResolutionsForConditions(conditionIds: string[]): Promise<Map<string, ResolutionInfo>> {
...
async function main() {
  ...
  console.log('UI ACTIVITY PNL SIMULATOR V3 - Cost Basis + Redemptions + Resolution Losses');
  console.log('='.repeat(100));
  console.log('');
  console.log('Algorithm: Average Cost Basis with:');
  console.log('  1. CLOB buys/sells');
  console.log('  2. PayoutRedemption events (sell at payout_price)');
  console.log('  3. Implicit resolution losses (remaining positions in resolved markets)');
  console.log('');
  ...
  for (const uiWallet of UI_BENCHMARK_WALLETS) {
    ...
    const activity = calculateActivityPnL(allEvents, resolutions);
  ...
  for (const { ui, activity, v9_pnl } of results) {
    ...
    console.log(`  Resolution losses:   ${activity.resolution_loss_events}`);
    console.log('');
  }
  ...
  console.log('SUMMARY: PnL Comparison (Activity+Redemptions+ResLosses vs V9 vs UI)');
  console.log('='.repeat(110));
  console.log('');
  console.log('| Wallet | UI PnL | Activity PnL | V9 Econ PnL | Activity Match | V9 Match | Redemptions | Res Losses |');
  console.log('|--------|--------|--------------|-------------|----------------|----------|-------------|------------|');
  ...
  for (const { ui, activity, v9_pnl } of results) {
    ...
    console.log(`| ${ui.label.padEnd(6)} | ${formatNumber(ui.profitLoss_all).padStart(8)} | ${formatNumber(activity.pnl_activity_total).padStart(12)} | ${formatNumber(v9_pnl).padStart(11)} | ${(activityPct.toFixed(1) + '%').padStart(14)} | ${(v9Pct.toFixed(1) + '%').padStart(8)} | ${String(activity.redemption_events).padStart(11)} | ${String(activity.resolution_loss_events).padStart(10)} |`);
  }
