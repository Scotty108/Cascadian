/**
 * Comprehensive V3 vs V5 PnL Accuracy Validation
 *
 * Tests V3 (average cost) vs V5 (V3 + splits/merges) against ALL known wallet PnL values.
 * This provides a complete picture of whether splits/merges handling improves accuracy.
 */
...
async function runComparison() {
  console.log('=== COMPREHENSIVE V3 vs V5 PnL ACCURACY COMPARISON ===\n');
  console.log('V3: Average cost basis (CLOB + Redemptions + Resolution)');
  console.log('V5: V3 + Splits + Merges (at $0.50 cost basis)\n');
  ...
  for (const w of ALL_KNOWN_WALLETS) {
    ...
    try {
      ...
      results.push({
        ...
        transfers: v5.transfers_in_count,
      });
  ...
  console.log(
    '| # | Wallet       | UI PnL    | V3 PnL    | V3 Err  | V5 PnL    | V5 Err  | Better | Splits | Merges |'
  );
  console.log(
    '|---|--------------|-----------|-----------|---------|-----------|---------|--------|--------|--------|'
  );
  ...
  for (let i = 0; i < results.length; i++) {
    const r = results[i];
    const improved = Math.abs(r.v5_error) < Math.abs(r.v3_error);
    const same = Math.abs(r.v5_error - r.v3_error) < 0.1;
    const better = same ? 'SAME' : improved ? 'V5 ✓' : 'V3';
    ...
    console.log(
      '| ' +
        ...
        '%'.padEnd(3) +
        ' | ' +
        fmt(r.v5_pnl).padEnd(9) +
        ' | ' +
        (r.v5_error >= 0 ? '+' : '') +
        r.v5_error.toFixed(1) +
        '%'.padEnd(3) +
        ' | ' +
        better.padEnd(6) +
        ' | ' +
        String(r.splits).padStart(6) +
        ' | ' +
  ...
  console.log('  V5: ' + v5SignOk + '/' + results.length + ' (' + ((v5SignOk / results.length) * 100).toFixed(1) + '%)');
  console.log('  Δ: ' + (v5SignOk - v3SignOk) + ' wallets');
  ...
  console.log('  Δ: ' + (v5AvgErr - v3AvgErr).toFixed(1) + '% (' + (v5AvgErr < v3AvgErr ? 'IMPROVED' : 'WORSE') + ')');

  console.log('\nMEDIAN ABSOLUTE ERROR:');
  console.log('  V3: ' + v3Median.toFixed(1) + '%');
  console.log('  V5: ' + v5Median.toFixed(1) + '%');
  console.log('  Δ: ' + (v5Median - v3Median).toFixed(1) + '% (' + (v5Median < v3Median ? 'IMPROVED' : 'WORSE') + ')');

  // Improvement count
  const improved = results.filter((r) => Math.abs(r.v5_error) < Math.abs(r.v3_error) - 0.1).length;
  const same = results.filter((r) => Math.abs(Math.abs(r.v5_error) - Math.abs(r.v3_error)) <= 0.1).length;
  const worse = results.filter((r) => Math.abs(r.v5_error) > Math.abs(r.v3_error) + 0.1).length;

  console.log('\nIMPROVEMENT COUNT:');
  console.log('  V5 better: ' + improved + ' wallets (' + ((improved / results.length) * 100).toFixed(0) + '%)');
  ...
  for (const b of buckets) {
    ...
    console.log(
      '  Within ' +
        ...
        results.length +
        ' (' +
        ((v5Count / results.length) * 100).toFixed(0) +
        '%)'
    );
  }
  ...
  const v5Exact = results.filter((r) => Math.abs(r.v5_error) < 1).length;
  console.log('\nEXACT MATCHES (<1% error):');
  console.log('  V3: ' + v3Exact + ' wallets');
  console.log('  V5: ' + v5Exact + ' wallets');

  // Splits/Merges correlation
  console.log('\n=== SPLITS/MERGES CORRELATION ===\n');
  const walletsWithSplits = results.filter((r) => r.splits > 0 || r.merges > 0);
  const walletsWithoutSplits = results.filter((r) => r.splits === 0 && r.merges === 0);

  console.log('Total splits: ' + results.reduce((s, r) => s + r.splits, 0).toLocaleString());
  console.log('Total merges: ' + results.reduce((s, r) => s + r.merges, 0).toLocaleString());
  console.log('Wallets with splits/merges: ' + walletsWithSplits.length + '/' + results.length);

  if (walletsWithSplits.length > 0) {
    const improvedWithSplits = walletsWithSplits.filter(
      (r) => Math.abs(r.v5_error) < Math.abs(r.v3_error)
    ).length;
    const v3AvgErrWithSplits =
      walletsWithSplits.reduce((s, r) => s + Math.abs(r.v3_error), 0) / walletsWithSplits.length;
    const v5AvgErrWithSplits =
      walletsWithSplits.reduce((s, r) => s + Math.abs(r.v5_error), 0) / walletsWithSplits.length;

    console.log(
      '\nWallets WITH splits/merges (' + walletsWithSplits.length + '):'
    );
    console.log('  V5 better: ' + improvedWithSplits + '/' + walletsWithSplits.length);
    console.log('  V3 avg error: ' + v3AvgErrWithSplits.toFixed(1) + '%');
    console.log('  V5 avg error: ' + v5AvgErrWithSplits.toFixed(1) + '%');
    console.log(
      '  Error reduction: ' +
        (v3AvgErrWithSplits - v5AvgErrWithSplits).toFixed(1) +
        ' percentage points'
    );
  }

  if (walletsWithoutSplits.length > 0) {
    const improvedWithoutSplits = walletsWithoutSplits.filter(
      (r) => Math.abs(r.v5_error) < Math.abs(r.v3_error)
    ).length;
    console.log(
      '\nWallets WITHOUT splits/merges (' + walletsWithoutSplits.length + '):'
    );
    console.log('  V5 better: ' + improvedWithoutSplits + '/' + walletsWithoutSplits.length);
  }

  // Top improvements
  console.log('\n=== TOP 10 IMPROVEMENTS (V5 vs V3) ===\n');
  const sortedByImprovement = [...results]
    .map((r) => ({ ...r, delta: Math.abs(r.v3_error) - Math.abs(r.v5_error) }))
    .filter((r) => r.delta > 0)
  ...
  const regressions = [...results]
    .map((r) => ({ ...r, delta: Math.abs(r.v5_error) - Math.abs(r.v3_error) }))
    .filter((r) => r.delta > 0.1)
    .sort((a, b) => b.delta - a.delta)
    .slice(0, 10);

  if (regressions.length > 0) {
    console.log('\n=== TOP 10 REGRESSIONS (V5 worse than V3) ===\n');
  ...
  // Final verdict
  console.log('\n=== FINAL VERDICT ===\n');
  if (v5Median < v3Median && v5AvgErr < v3AvgErr) {
    const medianImprovement = ((v3Median - v5Median) / v3Median) * 100;
    console.log('✅ V5 (Splits/Merges) IMPROVES accuracy');
    console.log(
      '   Median error: ' +
        v3Median.toFixed(1) +
        '% → ' +
        v5Median.toFixed(1) +
        '% (-' +
        medianImprovement.toFixed(0) +
        '%)'
    );
    console.log('   Mean error: ' + v3AvgErr.toFixed(1) + '% → ' + v5AvgErr.toFixed(1) + '%');
    console.log('\n   RECOMMENDATION: Promote V5 to production');
  } else if (
    Math.abs(v5Median - v3Median) < 1 &&
    Math.abs(v5AvgErr - v3AvgErr) < 1
  ) {
    console.log('➡️ V5 (Splits/Merges) has SIMILAR accuracy to V3');
    console.log(
      '   Median error: V3=' + v3Median.toFixed(1) + '%, V5=' + v5Median.toFixed(1) + '%'
    );
    console.log(
      '   Mean error: V3=' + v3AvgErr.toFixed(1) + '%, V5=' + v5AvgErr.toFixed(1) + '%'
    );
    console.log(
      '\n   RECOMMENDATION: V5 may help for specific wallet types but is not a general improvement.'
    );
  } else {
    console.log('❌ V5 (Splits/Merges) is WORSE than V3');
