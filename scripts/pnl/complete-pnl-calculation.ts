/**
 * Complete PnL calculation with proper weighted average and resolution handling
 */
...
async function main() {
  console.log('=== COMPLETE PNL CALCULATION ===');
  ...
  const resolutionMap = new Map<string, { payouts: number[]; denom: number }>();
  for (const row of await resolutions.json() as Array<{
    ...
    resolutionMap.set(row.condition_id, {
      ...
      denom: parseInt(row.payout_denominator),
    });
  }
  ...
  for (const pos of openPositions) {
    ...
    if (res) {
      const payoff = res.payouts[cond.outcome_index] / res.denom;
      currentValue = pos.amount * payoff;
      status = payoff > 0 ? 'WON' : 'LOST';
    } else {
