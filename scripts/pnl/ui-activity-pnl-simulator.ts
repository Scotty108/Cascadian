/**
 * UI Activity PnL Simulator
 *
 * Implements a cost-basis realized PnL engine that attempts to match
 * the Polymarket UI's "Profit/Loss" metric.
 *
 * Algorithm: Average Cost Basis
 * - Track position cost and qty per outcome
 * - On sell: realized_pnl = (sell_price - avg_cost) × qty_sold
 *
 * Usage: npx tsx scripts/pnl/ui-activity-pnl-simulator.ts
 */
...
async function getFillsForWallet(wallet: string): Promise<Fill[]> {
  const query = `
    SELECT
      m.condition_id,
      m.outcome_index,
...
function calculateActivityPnL(fills: Fill[]): WalletMetrics {
  // State per outcome (condition_id + outcome_index)
  const outcomeStates = new Map<string, OutcomeState>();

  const getKey = (fill: Fill): string =>
    `${fill.condition_id}_${fill.outcome_index}`;

  let volume_traded = 0;

  // Process fills in time order (already sorted)
  for (const fill of fills) {
    const key = getKey(fill);
...
async function main() {
  ...
  console.log('UI ACTIVITY PNL SIMULATOR - Cost Basis Algorithm');
  console.log('='.repeat(100));
  console.log('');
  console.log('Algorithm: Average Cost Basis');
  console.log('  - On buy: position_cost += usdc, position_qty += tokens');
  console.log('  - On sell: realized_pnl += (sell_price - avg_cost) × qty_sold');
  console.log('');
  ...
  for (const uiWallet of UI_BENCHMARK_WALLETS) {
    ...
    // Get fills and compute activity PnL
    const fills = await getFillsForWallet(uiWallet.wallet);
    const activity = calculateActivityPnL(fills);
  ...
  for (const { ui, activity, v9_pnl } of results) {
    ...
    console.log(`  Total fills:      ${activity.total_fills}`);
    console.log('');
  }
  ...
  console.log('\n');
  console.log('='.repeat(100));
  console.log('SUMMARY: PnL Comparison (Activity vs V9 vs UI)');
  console.log('='.repeat(100));
  console.log('');
  console.log('| Wallet | UI PnL | Activity PnL | V9 Econ PnL | Activity Match | V9 Match |');
  console.log('|--------|--------|--------------|-------------|----------------|----------|');
  ...
  for (const { ui, activity, v9_pnl } of results) {
    ...
    console.log(`| ${ui.label.padEnd(6)} | ${formatNumber(ui.profitLoss_all).padStart(8)} | ${formatNumber(activity.pnl_activity_total).padStart(12)} | ${formatNumber(v9_pnl).padStart(11)} | ${(activityPct.toFixed(1) + '%').padStart(14)} | ${(v9Pct.toFixed(1) + '%').padStart(8)} |`);
  }
