#!/usr/bin/env npx tsx
/**
 * Create materialized deduplicated table in BATCHES
 * Avoids memory limits by processing data in chunks
 */
...
async function createMaterializedDedupedBatched() {
  console.log('🔨 Creating materialized deduplicated table in batches...\n');
  ...
  // Insert in batches by first hex character of tx_hash
  console.log('3️⃣ Inserting data in 16 batches (by tx_hash prefix)...');
  const hexChars = '0123456789abcdef'.split('');

  for (let i = 0; i < hexChars.length; i++) {
    const char = hexChars[i];
    console.log(`   Batch ${i + 1}/16: tx_hash starting with '${char}'...`);
...
createMaterializedDedupedBatched()
  .then(() => {
    console.log('\n🎉 Done!');
    process.exit(0);
  })
