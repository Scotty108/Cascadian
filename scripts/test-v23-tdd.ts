/**
 * V23 TDD Test Suite
 * Tests V23 (Fully Local) against all 15 test wallets
 */
import { config } from 'dotenv';
import { resolve } from 'path';
config({ path: resolve(process.cwd(), '.env.local') });

import { getWalletPnLV23 } from '../lib/pnl/pnlEngineV23';
...
async function runTests() {
  console.log('=== V23 FULLY LOCAL ENGINE TEST SUITE ===\n');
  console.log('Name           | V23 Local    | API (truth)  | Diff      | Open | Status');
  console.log('---------------|--------------|--------------|-----------|------|-------');
  ...
  for (const w of TEST_WALLETS) {
    try {
      const [v23Result, apiPnl] = await Promise.all([
        getWalletPnLV23(w.wallet),
        fetchApiPnl(w.wallet)
      ]);

      const v23Total = v23Result.totalPnl;
      const apiTotal = apiPnl || 0;
      const absDiff = Math.abs(v23Total - apiTotal);
      const pctDiff = apiTotal !== 0 ? (absDiff / Math.abs(apiTotal)) * 100 : (v23Total === 0 ? 0 : 999);
      ...
      if (match) {
        ...
        failed++;
        failing.push(w.name + ': V23=' + v23Total.toFixed(0) + ' API=' + apiTotal.toFixed(0) + ' (' + pctDiff.toFixed(1) + '%)');
      }
      ...
      console.log(
        w.name.padEnd(14) + ' | $' +
        v23Total.toFixed(0).padStart(11) + ' | $' +
        apiTotal.toFixed(0).padStart(11) + ' | ' +
        diffStr.padStart(9) + ' | ' +
        v23Result.openPositionCount.toString().padStart(4) + ' | ' +
        status
      );
    } catch (error) {
      failed++;
