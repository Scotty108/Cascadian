#!/usr/bin/env npx tsx
/**
 * CHUNKED Wallet Metrics Rebuild - Uses Canonical P&L Pipeline
 *
 * KEY FIX: Uses trade_cashflows_v3 instead of trades_raw
 * IMPROVEMENT: Processes wallets in 5K batches to avoid connection overflow
 *
...
const CHUNK_SIZE = 5000; // Process 5K wallets at a time

async function main() {
  const ch = getClickHouseClient();

  console.log('\n' + '═'.repeat(100));
  console.log('WALLET METRICS REBUILD - CHUNKED (Using Canonical P&L Pipeline)');
  ...
  try {
    ...
    const walletListQuery = `
      SELECT DISTINCT lower(wallet) as wallet_address
      FROM default.trade_cashflows_v3
      ORDER BY wallet_address
    `;

    const walletResult = await ch.query({ query: walletListQuery, format: 'JSONEachRow' });
    const allWallets = await walletResult.json<{ wallet_address: string }[]>();
    ...
    // Step 4: Process each window in chunks
    console.log('3️⃣  Populating windows in chunks...\n');

    for (const window of windows) {
      console.log(`   Window: ${window.name} (≥ ${window.dateStart})`);

      const chunks = Math.ceil(totalWallets / CHUNK_SIZE);
      let processedWallets = 0;

      for (let chunkIdx = 0; chunkIdx < chunks; chunkIdx++) {
        const startIdx = chunkIdx * CHUNK_SIZE;
        const endIdx = Math.min(startIdx + CHUNK_SIZE, totalWallets);
        const chunkWallets = allWallets.slice(startIdx, endIdx);

        // Build wallet list for IN clause
        const walletList = chunkWallets
        ...
        const insertSQL = `
          ...
          FROM (
            ...
            FROM (
              SELECT unnest([${walletList}]) as wallet_address
            )
          ) all_wallets
          LEFT JOIN (
            SELECT
            ...
            INNER JOIN (
              ...
              WHERE block_time >= '${window.dateStart}'
                ...
                AND lower(wallet) IN (${walletList})
        ...
        processedWallets += chunkWallets.length;
        const progress = ((processedWallets / totalWallets) * 100).toFixed(1);

        console.log(`     Chunk ${chunkIdx + 1}/${chunks}: ${chunkWallets.length} wallets (${progress}% - ${elapsed}s)`);
      }

      console.log(`   ✅ ${window.name} complete: ${processedWallets.toLocaleString()} wallets\n`);
    }
    ...
    console.log('═'.repeat(100));
    console.log(`\n✅ Processed ${totalWallets.toLocaleString()} wallets in ${CHUNK_SIZE.toLocaleString()}-wallet chunks`);
