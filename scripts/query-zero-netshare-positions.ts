...
async function main() {
  const query = `
    ...
    SELECT
      count() AS zero_net_positions,
      sumIf(cashflow_micro, abs(net_shares) <= 0.0001) AS total_cashflow_micro,
      sumIf(abs(cashflow_micro), abs(net_shares) <= 0.0001) AS abs_cashflow_micro
    FROM aggregated
    WHERE abs(net_shares) <= 0.0001 AND abs(cashflow_micro) > 0
  `;
