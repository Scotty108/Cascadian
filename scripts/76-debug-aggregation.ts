...
// Check resolved_only CTE directly
const query = `
...
async function main() {
  console.log(`Debugging aggregation for wallet ${WALLET.substring(0, 10)}...\n`);
  ...
  console.log('Resolved-Only CTE Results:');
  ...
  if (parseFloat(row.total_settlement || 0) === 0) {
    console.log('\n❌ Settlement is STILL $0 in the CTE!');
    console.log('   This suggests the issue is in the CTE creation, not the final SELECT.');
  } else {
    console.log('\n✅ Settlement is NON-ZERO in the CTE!');
    console.log('   The issue must be in the final SELECT subqueries.');
  }
}
