...
import { getWalletPnLV26 } from '../lib/pnl/pnlEngineV26';
...
async function runTests() {
  console.log('=== V26 SPLIT-AWARE ENGINE TEST SUITE ===\n');
  console.log('Name           | V26 Local    | API (truth)  | Diff      | Open | Splits | Status');
  console.log('---------------|--------------|--------------|-----------|------|--------|-------');
  ...
  for (const w of TEST_WALLETS) {
    try {
      const [v26Result, apiPnl] = await Promise.all([
        getWalletPnLV26(w.wallet),
        fetchApiPnl(w.wallet)
      ]);

      const v26Total = v26Result.totalPnl;
      const apiTotal = apiPnl || 0;
      const absDiff = Math.abs(v26Total - apiTotal);
      const pctDiff = apiTotal !== 0 ? (absDiff / Math.abs(apiTotal)) * 100 : (v26Total === 0 ? 0 : 999);
      ...
      console.log(
        w.name.padEnd(14) + ' | $' +
        v26Total.toFixed(0).padStart(11) + ' | $' +
        apiTotal.toFixed(0).padStart(11) + ' | ' +
        diffStr.padStart(9) + ' | ' +
        v26Result.openPositionCount.toString().padStart(4) + ' | ' +
        v26Result.splitPatternCount.toString().padStart(6) + ' | ' +
        status
      );
    } catch (error) {
      failed++;
