#!/usr/bin/env npx tsx
/**
 * FIFO Refresh - Process ALL pending resolved conditions
 *
 * Processes all conditions resolved in last 48 hours that aren't yet in pm_trade_fifo_roi_v3
 */
...
async function refreshFifo() {
  console.log('🔄 FIFO Refresh - Processing all pending conditions\n');
  const startTime = Date.now();

  // Get all unprocessed conditions
  ...
  if (conditions.length === 0) {
    console.log('✅ Nothing to process - FIFO table is up to date');
    return;
  }
  ...
  for (let i = 0; i < conditions.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      if (batchNum % 10 === 0 || batchNum === totalBatches) {
        console.log(`   Batch ${batchNum}/${totalBatches} - ${processed} conditions processed`);
      }
    } catch (err: any) {
      errors++;
      console.error(`   Batch ${batchNum} error: ${err.message.slice(0, 80)}`);
    }
  }

  const elapsed = ((Date.now() - startTime) / 1000 / 60).toFixed(1);

  console.log(`\n✅ FIFO Refresh Complete! (${elapsed} min)`);
  console.log(`   Processed: ${processed} conditions`);
  console.log(`   Errors: ${errors}`);

  // Check new freshness
  const freshness = await clickhouse.query({
    query: `SELECT max(resolved_at) as newest FROM pm_trade_fifo_roi_v3`,
    format: 'JSONEachRow'
  });
  const f = (await freshness.json() as any)[0];
  console.log(`   Newest resolved: ${f.newest}`);
}

refreshFifo().catch(console.error);
