#!/usr/bin/env npx tsx
/**
 * FIFO Position Lifecycle Worker - AUDIT VALIDATION
 *
 * Fast validation on 23 audit wallets only.
 * Adds price fallback: CLOB mid → last trade price.
 */
...
async function main() {
  ...
  console.log('\n═══════════════════════════════════════════════════════════════════════════════');
  console.log('FIFO POSITION LIFECYCLE WORKER - AUDIT VALIDATION');
  console.log('═══════════════════════════════════════════════════════════════════════════════\n');
  ...
  // Step 2: Fetch audit wallet keys directly
  console.log('Step 2: Fetching audit wallet keys...');
  ...
  // Show sample metrics
  console.log('Sample Wallet Time Metrics:');
  const sample = await ch.query({
    query: `
      SELECT
        ...
        positions_closed,
        positions_open,
        round(avg_hold_hours / 24, 1) AS avg_hold_days,
        round(pct_held_gt_7d, 1) AS pct_long_term,
        round(pct_held_lt_1d, 1) AS pct_intraday
  ...
  sampleData.forEach((w, i) => {
    console.log(`  ${i + 1}. ${w.wallet.substring(0, 12)}... - ${w.positions_total} total (${w.positions_closed} closed, ${w.positions_open} open), avg ${w.avg_hold_days} days, ${w.pct_long_term}% long-term, ${w.pct_intraday}% intraday`);
  });
  ...
  console.log('');
  console.log('✅ COMPLETE - AUDIT VALIDATION');
  console.log('');
  console.log('Next: Compare with Polymarket to validate accuracy');
  console.log('');
