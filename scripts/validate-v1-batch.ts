/**
 * V1 BATCH Validation - 50 wallets in ONE query
 * Uses batch processing for massive speedup
 */
...
async function main() {
  ...
  const outputFile = `scripts/v1-batch-${new Date().toISOString().replace(/:/g, '-').slice(0, 19)}.json`;

  console.log('=== V1 BATCH VALIDATION (50 wallets) ===');
  console.log(`Started: ${new Date().toISOString()}`);
  console.log(`Output: ${outputFile}\n`);

  // Step 1: Batch calculate all PnL in ONE query
  console.log('Calculating PnL for all 50 wallets in batch...');
