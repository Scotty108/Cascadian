...
async function runTests() {
  console.log('=== V28 PURE AVGPRICE ENGINE TEST SUITE ===\n');
  console.log('Name           | V28 Local    | API (truth)  | Diff      | Open | Status');
  console.log('---------------|--------------|--------------|-----------|------|-------');
  ...
  for (const w of TEST_WALLETS) {
    try {
      ...
      if (match) {
        ...
        failed++;
        failing.push(w.name + ': V28=' + v28Total.toFixed(0) + ' API=' + apiTotal.toFixed(0) + ' (' + pctDiff.toFixed(1) + '%)');
      }
      ...
      console.log(
        w.name.padEnd(14) + ' | $' +
        v28Total.toFixed(0).padStart(11) + ' | $' +
        apiTotal.toFixed(0).padStart(11) + ' | ' +
        diffStr.padStart(9) + ' | ' +
        v28Result.openPositionCount.toString().padStart(4) + ' | ' +
        status
      );
    } catch (error) {
      failed++;
