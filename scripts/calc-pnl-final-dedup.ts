...
async function check() {
  // Correct dedup: GROUP BY (tx, side, token_id, token_amount)
  const q = `
    SELECT
      token_id,
  ...
  let totalPnl = 0;
  let wonCount = 0, lostCount = 0, unresolvedCount = 0;
  let wonProfit = 0, lostCost = 0;

  console.log('PNL WITH CORRECT DEDUP (tx, side, token_id, amount):');
  ...
  console.log(`\n... ${lostCount} lost positions totaling -$${lostCost.toFixed(2)}`);
