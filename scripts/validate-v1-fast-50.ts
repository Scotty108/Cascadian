/**
 * V1 FAST Validation - 50 wallets
 * Uses V1 for PnL + diagnostics for confidence (no V1+ for speed)
 */

import { config } from 'dotenv';
config({ path: '.env.local' });

import { getWalletPnLV1, getWalletDiagnostics } from '../lib/pnl/pnlEngineV1';
...
function calculateConfidence(diag: any): { confidence: string; reasons: string[]; wouldBenefitFromV1Plus: boolean } {
  const reasons: string[] = [];
  let confidence: 'high' | 'medium' | 'low' = 'high';

  // Would V1+ help?
  const wouldBenefitFromV1Plus = diag.phantomTokens > 100 && diag.negRiskTokens > 100;

  // LOW: Unexplained phantom
  if (diag.unexplainedPhantom > 1000) {
    reasons.push(`${Math.round(diag.unexplainedPhantom)} unexplained phantom`);
    confidence = 'low';
  }

  // LOW: High phantom % without external source
  if (diag.phantomPercent > 50 && diag.negRiskConversions === 0 && diag.ctfSplitMergeCount === 0) {
    reasons.push(`${diag.phantomPercent}% phantom (no external source)`);
    confidence = 'low';
  }

  // MEDIUM: Many open positions
  if (diag.openPositions > 10 && confidence !== 'low') {
    reasons.push(`${diag.openPositions} open positions`);
    if (confidence === 'high') confidence = 'medium';
  }

  // MEDIUM: Low resolved %
  if (diag.resolvedPositionPct < 50 && diag.totalPositions > 5 && confidence !== 'low') {
    reasons.push(`${diag.resolvedPositionPct}% resolved`);
    if (confidence === 'high') confidence = 'medium';
  }

  if (reasons.length === 0) {
    reasons.push('No risk factors');
  }

  return { confidence, reasons, wouldBenefitFromV1Plus };
}
...
interface Result {
  ...
  confidence: string;
  reasons: string[];
  wouldBenefitFromV1Plus: boolean;
  status: string;
  elapsedSec: number;
  error?: string;
}
...
async function testWallet(w: { wallet: string; cohort: string }): Promise<Result> {
  ...
  try {
    // Run API and diagnostics in parallel
    const [api, diag] = await Promise.all([
      getApiPnL(w.wallet),
      getWalletDiagnostics(w.wallet),
    ]);
    ...
    // Calculate confidence from diagnostics
    const { confidence, reasons, wouldBenefitFromV1Plus } = calculateConfidence(diag);

    const gap = Math.abs(api - v1);
    ...
    return {
      ...
      gap: NaN,
      confidence: 'unknown',
      reasons: [e.message?.slice(0, 60)],
      wouldBenefitFromV1Plus: false,
      status: 'ERR',
      elapsedSec: Math.round((Date.now() - start) / 1000),
...
async function main() {
  ...
  const outputFile = `scripts/v1-fast-${new Date().toISOString().replace(/:/g, '-').slice(0, 19)}.json`;

  console.log('=== V1 FAST VALIDATION (50 wallets) ===');
  ...
  for (const w of COHORT) {
    ...
    const v1PlusFlag = result.wouldBenefitFromV1Plus ? ' V1+?' : '';
    console.log(`${result.status.padEnd(8)} (${result.elapsedSec}s) Gap=$${result.gap} [${result.confidence}]${v1PlusFlag}`);
  ...
  const wouldBenefit = valid.filter(r => r.wouldBenefitFromV1Plus).length;

  console.log('\n=== SUMMARY ===');
  console.log(`PASS (≤$10): ${pass}/${valid.length} (${(pass/valid.length*100).toFixed(1)}%)`);
  console.log(`CLOSE (≤$100): ${close}/${valid.length}`);
  console.log(`FAIL (>$100, high/med conf): ${fail}/${valid.length}`);
  console.log(`LOW-OK (low conf, ≤$100): ${lowOk}/${valid.length}`);
  console.log(`LOW-FAIL (low conf, >$100): ${lowFail}/${valid.length}`);
  console.log(`ERRORS: ${errors}`);
  console.log('');
  console.log(`HIGH CONFIDENCE accuracy: ${highConfPass}/${highConf.length} wallets within $10`);
  console.log(`Would benefit from V1+: ${wouldBenefit} wallets`);
