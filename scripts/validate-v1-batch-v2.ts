/**
 * V1 BATCH V2 Validation - 50 wallets with ERC1155 transfers included
 * Adds direct ERC1155 transfers as token source to fix phantom positions
 */
...
async function batchPnLV2(wallets: string[]): Promise<Map<string, number>> {
  const walletList = wallets.map(w => "'" + w + "'").join(',');

  // V2: Now includes ERC1155 direct transfers as a token source
  const query = `
    WITH
      wallet_list AS (
        SELECT arrayJoin([${walletList}]) as wallet
      ),
      -- Self-fill detection (same wallet is both maker and taker in same tx)
      self_fills AS (
      ...
      -- CLOB positions (exclude maker side of self-fills)
      clob_pos AS (
        SELECT
      ...
      -- NEW: ERC1155 direct transfers (tokens received from other wallets)
      -- Convert hex token_id to decimal to join with mapping table
      erc1155_transfers AS (
        SELECT
          lower(e.to_address) as wallet,
          m.condition_id,
          m.outcome_index,
          -- Tokens received (inflow)
          sum(reinterpretAsUInt64(reverse(unhex(substring(e.value, 3))))) / 1e6 as received_tokens
        ...
        WHERE lower(e.to_address) IN (SELECT wallet FROM wallet_list)
          AND lower(e.from_address) NOT IN ('0x0000000000000000000000000000000000000000', '')
          AND e.is_deleted = 0
          AND m.condition_id != ''
        GROUP BY lower(e.to_address), m.condition_id, m.outcome_index
      ),
      -- ERC1155 transfers OUT (tokens sent to other wallets)
      erc1155_transfers_out AS (
        SELECT
          lower(e.from_address) as wallet,
          m.condition_id,
          m.outcome_index,
          sum(reinterpretAsUInt64(reverse(unhex(substring(e.value, 3))))) / 1e6 as sent_tokens
        FROM pm_erc1155_transfers e
        JOIN pm_token_to_condition_map_v5 m
          ON toString(reinterpretAsUInt256(reverse(unhex(substring(e.token_id, 3))))) = m.token_id_dec
        WHERE lower(e.from_address) IN (SELECT wallet FROM wallet_list)
          AND lower(e.to_address) NOT IN ('0x0000000000000000000000000000000000000000', '')
          AND e.is_deleted = 0
          AND m.condition_id != ''
        GROUP BY lower(e.from_address), m.condition_id, m.outcome_index
      ),
      -- Combine all token sources
      combined AS (
        SELECT
          COALESCE(c.wallet, f.wallet, n.wallet, e.wallet, o.wallet) as wallet,
          COALESCE(c.condition_id, f.condition_id, n.condition_id, e.condition_id, o.condition_id) as condition_id,
          COALESCE(c.outcome_index, f.outcome_index, n.outcome_index, e.outcome_index, o.outcome_index) as outcome_index,
          -- Net tokens = CLOB + CTF + NegRisk + ERC1155_in - ERC1155_out
          COALESCE(c.clob_tokens, 0) + COALESCE(f.ctf_tokens, 0) + COALESCE(n.nr_tokens, 0)
            + COALESCE(e.received_tokens, 0) - COALESCE(o.sent_tokens, 0) as net_tokens,
        ...
        FULL OUTER JOIN erc1155_transfers e ON
          COALESCE(c.wallet, f.wallet, n.wallet) = e.wallet
          AND COALESCE(c.condition_id, f.condition_id, n.condition_id) = e.condition_id
          AND COALESCE(c.outcome_index, f.outcome_index, n.outcome_index) = e.outcome_index
        FULL OUTER JOIN erc1155_transfers_out o ON
          COALESCE(c.wallet, f.wallet, n.wallet, e.wallet) = o.wallet
          AND COALESCE(c.condition_id, f.condition_id, n.condition_id, e.condition_id) = o.condition_id
          AND COALESCE(c.outcome_index, f.outcome_index, n.outcome_index, e.outcome_index) = o.outcome_index
      ),
...
async function main() {
  ...
  const outputFile = `scripts/v1-batch-v2-${new Date().toISOString().replace(/:/g, '-').slice(0, 19)}.json`;

  console.log('=== V1 BATCH V2 VALIDATION (50 wallets + ERC1155) ===');
  console.log(`Started: ${new Date().toISOString()}`);
  console.log(`Output: ${outputFile}\n`);

  // Step 1: Batch calculate all PnL in ONE query
  console.log('Calculating PnL for all 50 wallets (with ERC1155 transfers)...');
  const batchStart = Date.now();
  const pnlMap = await batchPnLV2(COHORT);
  const batchTime = ((Date.now() - batchStart) / 1000).toFixed(1);
  console.log(`Batch query completed in ${batchTime}s\n`);

  // Step 2: Get API values (with rate limiting)
  console.log('Fetching API values...');
  const results: any[] = [];
  ...
  for (let i = 0; i < COHORT.length; i++) {
    ...
    results.push({ wallet, api: Math.round(api * 100) / 100, calculated, gap: Math.round(gap * 100) / 100, status });
  ...
  // Compare with V1 batch
  console.log('\n=== IMPROVEMENT VS V1 ===');
  const v1Failures = ['0xeb71ad2f', '0x90389ac0', '0x093f608f', '0xe272a0fb', '0x59207e5e',
                      '0x4e3aa655', '0x98fb352a', '0x29569f0b', '0x4bdb41f9', '0x6a315959', '0x060be258'];
  for (const fail of v1Failures) {
    const r = results.find(x => x.wallet.startsWith(fail));
    if (r) {
      console.log(`${fail}: Gap=$${r.gap.toFixed(2)} (${r.status})`);
    }
  }

  console.log(`\nTotal time: ${((Date.now() - startTime) / 1000).toFixed(1)}s`);
  console.log(`  - Batch PnL: ${batchTime}s`);
  console.log(`  - API calls: ${((Date.now() - startTime) / 1000 - parseFloat(batchTime)).toFixed(1)}s`);

  // Save results
  fs.writeFileSync(outputFile, JSON.stringify({ timestamp: new Date().toISOString(), batchTime, results }, null, 2));
