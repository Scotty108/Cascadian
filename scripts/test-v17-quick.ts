...
async function runTests() {
  console.log('=== V17 TDD TEST SUITE ===\n');
  console.log('Name           | V17 Local    | API (truth)  | Diff      | Compl | Status');
  console.log('---------------|--------------|--------------|-----------|-------|-------');
  ...
  for (const w of TEST_WALLETS) {
    try {
      const [v19Result, apiPnl] = await Promise.all([getWalletPnLV17(w.wallet), fetchApiPnl(w.wallet)]);
      ...
      if (match) passed++; else { failed++; failing.push(w.name + ': V17=$' + v19Total.toFixed(0) + ' API=$' + apiTotal.toFixed(0) + ' (' + pctDiff.toFixed(1) + '%)'); }
      const status = match ? 'PASS' : 'FAIL';
      const diffStr = pctDiff < 999 ? pctDiff.toFixed(1) + '%' : 'N/A';
      console.log(w.name.padEnd(14) + ' | $' + v19Total.toFixed(0).padStart(11) + ' | $' + apiTotal.toFixed(0).padStart(11) + ' | ' + diffStr.padStart(9) + ' | ' + v19Result.bundledTxCount.toString().padStart(5) + ' | ' + status);
    } catch (error) {
