#!/usr/bin/env npx tsx
/**
 * PHASE 2: Last 2 Days (Production Ready)
 *
...
async function rebuildLast2Days() {
  const startTime = Date.now();
  console.log('🔨 PHASE 2: Last 2 Days (Production Ready)\n');
  ...
  console.log('2️⃣ Finding active wallets (last 2 days)...');
  await clickhouse.command({
    query: `CREATE TABLE IF NOT EXISTS temp_active_wallets_2d (wallet LowCardinality(String)) ENGINE = Memory`
  });
  await clickhouse.command({ query: 'TRUNCATE TABLE temp_active_wallets_2d' });
  await clickhouse.query({
    query: `INSERT INTO temp_active_wallets_2d SELECT DISTINCT wallet FROM pm_trade_fifo_roi_v3 WHERE entry_time >= now() - INTERVAL 2 DAY`
  });
  ...
  // Step 6: Early selling check
  console.log('6️⃣ Early Selling Check...');
  ...
  if (total === unique) {
    ...
    console.log(`\n📋 Next: Schedule Phase 3 (full 192M row backfill) overnight`);
  } else {
