/**
 * Find Human Copy-Trading Candidates - V6
 * FIXED: Only filter out wallet-to-wallet transfers, NOT burns/redemptions
 *
 * Filters:
 * 1. PnL > 0 (profitable)
 ...
 * 5. NO wallet-to-wallet CTF transfers (but burns/redemptions are OK!)
 */
...
async function main() {
  console.log('=== FINDING HUMAN COPY-TRADING CANDIDATES V6 ===\n');
  console.log('FIXED: Allow redemptions (burns), only filter wallet-to-wallet transfers\n');
  console.log('Filters: PnL > 0, 10+ trades, active 10 days, max 200/day, no W2W transfers\n');
  ...
  // Step 3: Get wallets with WALLET-TO-WALLET transfers (NOT burns/redemptions)
  // Burns are to 0x0000...0000 (legitimate redemptions)
  // Mints are from 0x0000...0000 (internal system)
  // We only want to exclude actual wallet-to-wallet transfers
  console.log('Step 3: Finding wallets with wallet-to-wallet transfers (to exclude)...');
  console.log('        (Burns/redemptions to 0x0 are OK!)');

  const w2wQuery = await clickhouse.query({
    query: `
      SELECT DISTINCT wallet FROM (
        -- Wallets that SENT tokens to other wallets (not burns)
        ...
        -- Wallets that RECEIVED tokens from other wallets (not mints)
  ...
  const w2wWallets = new Set((await w2wQuery.json() as any[]).map(w => w.wallet));
  console.log(`Wallets with wallet-to-wallet transfers: ${w2wWallets.size.toLocaleString()}\n`);

  // Also get wallets that ONLY have burns (redemptions) - these are clean!
  const burnOnlyQuery = await clickhouse.query({
    query: `
      SELECT count() as burn_only_count
      FROM (
  ...
  const burnOnly = (await burnOnlyQuery.json() as any[])[0];
  console.log(`Wallets with ONLY burns (clean redemptions): ${burnOnly?.burn_only_count?.toLocaleString() || 'N/A'}\n`);
  ...
  console.log(`Filtered out - Has W2W transfers: ${w2wCount.toLocaleString()}`);

  console.log(`\n${'='.repeat(80)}`);
  console.log(`FINAL HUMAN CANDIDATES (no W2W transfers): ${humanCandidates.length.toLocaleString()}`);
  ...
  fs.writeFileSync('/tmp/human-candidates-v6.json', JSON.stringify(humanCandidates, null, 2));
  fs.writeFileSync('/tmp/copy-targets-v6.json', JSON.stringify(copyTargets, null, 2));

  console.log(`\nFull list saved to /tmp/human-candidates-v6.json`);
  console.log(`Copy targets saved to /tmp/copy-targets-v6.json`);

  // Summary comparison with V5
  console.log(`\n=== V6 vs V5 COMPARISON ===`);
  console.log(`V5 (no CTF at all):     15,715 candidates, 44 copy targets`);
  console.log(`V6 (allow redemptions): ${humanCandidates.length.toLocaleString()} candidates, ${copyTargets.length} copy targets`);
  console.log(`\nDifference: +${(humanCandidates.length - 15715).toLocaleString()} candidates we were incorrectly excluding!`);
}
