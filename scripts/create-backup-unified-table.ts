...
async function createBackup() {
  ...
  console.log('2️⃣ Creating backup table (this may take a few minutes)...');
  await clickhouse.command({
    query: `
      CREATE TABLE pm_trade_fifo_roi_v3_mat_unified_backup_20260129 AS
    ...
    `,
    clickhouse_settings: {
      max_execution_time: 3600,  // 1 hour for copy
    }
  });

  const elapsed = ((Date.now() - startTime) / 1000 / 60).toFixed(1);
  console.log(`   ✅ Backup created (${elapsed} minutes)\n`);
