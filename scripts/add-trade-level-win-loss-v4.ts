/**
 * Add trade-level win/loss counts v4
 * - Join trades with token map to get condition_id
...
async function getTradeStats(wallet: string): Promise<TradeStats> {
  const query = `
    ...
    trades AS (
      ...
      LEFT JOIN pm_token_to_condition_map_unified m ON t.token_id = m.token_id
    ...
    position_final AS (
      SELECT
        ...
        p.usdc_flow + (CASE
          WHEN r.payout IS NOT NULL THEN
            p.net_tokens * arrayElement(r.payout, toUInt8(p.outcome_index) + 1)
          ELSE 0
        END) as final_pnl
      FROM position_pnl p
      LEFT JOIN pm_condition_resolution_map r ON lower(p.condition_id) = lower(r.condition_id)
    )
    SELECT
      ...
      sum(trade_count) as total_classified,
  ...
  return rows[0] || { winning_trades: 0, losing_trades: 0, total_classified: 0, total_trades: 0 };
}
...
async function main() {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      console.error(`Error for ${wallet}:`, err.message?.slice(0, 100));
