#!/usr/bin/env npx tsx
/**
 * Deduplicate 10-Day Table (Parallel Workers)
 *
 * Splits deduplication across 10 parallel workers to stay under memory limit.
 * Each worker processes 10% of wallets (cityHash64(wallet) % 10).
 *
 * Expected:
 * - Memory per worker: ~1-2 GB (well under 10.8 GB limit)
 * - Total time: 10-15 min (parallel execution)
...
async function processWorkerBatch(workerId: number) {
  console.log(`\n[Worker ${workerId}] Processing wallet partition ${workerId}/${NUM_WORKERS}...\n`);
  ...
  console.log(`   ✅ Worker ${workerId} complete: ${rows.toLocaleString()} rows in ${elapsed} min\n`);
  return { workerId, rows, elapsed };
}

async function orchestrateParallelWorkers() {
  console.log('\n🚀 Launching Parallel Workers...\n');
  console.log(`   Workers: ${NUM_WORKERS} (processing 10% each)`);
  console.log('   Expected time: 10-15 min\n');

  const startTime = Date.now();

  // Launch all workers in parallel using Promise.all
  const workerPromises = [];
  for (let i = 0; i < NUM_WORKERS; i++) {
    workerPromises.push(processWorkerBatch(i));
  }
...
async function main() {
  // Check if running as individual worker
  if (WORKER_ID >= 0) {
    console.log(`🔧 Running as Worker ${WORKER_ID}/${NUM_WORKERS}`);
    await processWorkerBatch(WORKER_ID);
    process.exit(0);
  }

  // Otherwise, run as orchestrator
  console.log('🧹 DEDUPLICATE 10-DAY TABLE (Parallel Workers)');
  console.log('═'.repeat(70));
  console.log(`⏰ Started at: ${new Date().toLocaleString()}`);
  console.log(`📋 Table: ${OLD_TABLE}`);
  console.log(`👷 Workers: ${NUM_WORKERS} parallel workers (10% each)`);
  console.log('═'.repeat(70));
  ...
  try {
    ...
    const { totalRows, totalElapsed } = await orchestrateParallelWorkers();
    ...
    console.log(`   - Worker execution: ${totalElapsed} min (parallel)`);
    ...
    console.log('   5. Dedupe main table: npx tsx scripts/deduplicate-unified-main-parallel.ts');
    console.log('');
