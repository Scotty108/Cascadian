#!/usr/bin/env npx tsx
/**
 * Find ULTRA ACTIVE Traders (Last 3 Days)
 *
 * Filters:
 * - Active in last 3 days
 * - High win rate (>70%)
 * - HIGH MEDIAN ROI (>30%)
 * - Profitable overall (total PnL > $10k - lower threshold for recent activity)
 * - Meaningful sample size (30+ trades - lower for very active traders)
...
async function find3DayActiveTraders() {
  console.log('=== Finding ULTRA ACTIVE Traders (Last 3 days, 70%+ WR, 30%+ MEDIAN ROI) ===\n');
  ...
  const tradersResult = await clickhouse.query({
    query: `
      WITH wallet_stats AS (
        ...
        GROUP BY wallet
        HAVING total_trades >= 30          -- Lower threshold for very active traders
          AND total_pnl > 10000            -- Lower threshold ($10k) for recent activity
  ...
  if (traders.length === 0) {
    ...
    console.log('   Criteria: 30+ trades, >$10k profit, >70% win rate, >30% MEDIAN ROI, active in last 3 days');
    return;
  }

  console.log(`✓ Found ${traders.length} ULTRA ACTIVE traders (last 3 days)\n`);
  console.log('=== ULTRA ACTIVE TRADERS (Last 3 Days) ===\n');
  ...
  console.log('\n\n🎯 TOP RECOMMENDATIONS (ACTIVE LAST 3 DAYS):\n');
  ...
  console.log(`   Trades: ${mostActive.total_trades} | ${mostActive.trades_per_day}/day`);
  ...
  console.log(`   Median ROI: ${topProfit.median_roi_pct}% | Win Rate: ${topProfit.win_rate_pct}%`);
  console.log(`   Last: ${topProfit.hours_since_last}h ago\n`);

  // Most recent
  const mostRecent = traders.reduce((prev, curr) =>
    (curr.hours_since_last < prev.hours_since_last) ? curr : prev
  );
  console.log(`⏰ MOST RECENT:`);
  console.log(`   Wallet: ${mostRecent.wallet}`);
  console.log(`   Last: ${mostRecent.hours_since_last}h ago (${mostRecent.last_trade})`);
  console.log(`   Median ROI: ${mostRecent.median_roi_pct}% | Win Rate: ${mostRecent.win_rate_pct}%`);
  console.log(`   Profit: $${mostRecent.total_pnl.toLocaleString()}\n`);

  // Summary stats
  console.log('\n📊 COHORT STATS (ACTIVE LAST 3 DAYS):');
  ...
  console.log('\n\n📄 EXPORTING TO CSV...\n');
  const csv = [
    'wallet,total_trades,wins,losses,win_rate_pct,total_pnl,median_roi_pct,avg_roi_pct,roi_stddev,avg_position_size,total_volume,trades_per_day,avg_hold_hours,short_pct,last_trade,hours_since_last',
    ...traders.map(w =>
      `${w.wallet},${w.total_trades},${w.wins},${w.losses},${w.win_rate_pct},${w.total_pnl},${w.median_roi_pct},${w.avg_roi_pct},${w.roi_stddev},${w.avg_position_size},${w.total_volume},${w.trades_per_day},${w.avg_hold_hours},${w.short_pct},${w.last_trade},${w.hours_since_last}`
    )
  ].join('\n');

  const fs = require('fs');
  const csvPath = '/Users/scotty/Projects/Cascadian-app/ultra-active-3day-traders.csv';
  fs.writeFileSync(csvPath, csv);
  console.log(`✅ Exported ${traders.length} wallets to: ${csvPath}`);
}

find3DayActiveTraders().catch(e => {
