#!/usr/bin/env npx tsx
/**
 * Find ACTIVE Consistent High ROI Traders (MEDIAN not AVERAGE)
 *
 * Filters:
 * - High win rate (>70%)
...
async function findActiveConsistentROI() {
  console.log('=== Finding ACTIVE Consistent High ROI Traders (Last 10 days, 70%+ WR, 30%+ MEDIAN ROI) ===\n');
    ...
    query: `
      WITH wallet_stats AS (
        ...
        HAVING total_trades >= 50          -- Meaningful sample size
          ...
          AND days_since_last <= 10        -- ACTIVE IN LAST 10 DAYS!
      )
      SELECT
        wallet,
        total_trades,
  ...
  if (traders.length === 0) {
    ...
    console.log('   Criteria: 50+ trades, >$50k profit, >70% win rate, >30% MEDIAN ROI, active in last 10 days');
    return;
  }

  console.log(`✓ Found ${traders.length} ACTIVE consistent high ROI traders\n`);
  console.log('=== TOP ACTIVE CONSISTENT HIGH ROI TRADERS (MEDIAN ROI, last 10 days) ===\n');
  ...
  console.log('\n\n🎯 TOP RECOMMENDATIONS (ACTIVE LAST 10 DAYS):\n');
  ...
  console.log(`🔥 MOST ACTIVE (highest trade count):`);
  ...
  console.log(`   Last Active: ${mostRecent.days_since_last} days ago (${mostRecent.last_trade})`);
  console.log(`   Median ROI: ${mostRecent.median_roi_pct}% (avg: ${mostRecent.avg_roi_pct}%)`);
  console.log(`   Win Rate: ${mostRecent.win_rate_pct}% (${mostRecent.wins}W-${mostRecent.losses}L)`);
  console.log(`   Profit: $${mostRecent.total_pnl.toLocaleString()}`);
  console.log(`   Trades: ${mostRecent.total_trades}\n`);

  // Summary stats
  console.log('\n📊 COHORT STATS (ACTIVE LAST 10 DAYS):');
  ...
  console.log('\n\n📄 EXPORTING TO CSV...\n');
  const csv = [
    'wallet,total_trades,wins,losses,win_rate_pct,total_pnl,median_roi_pct,avg_roi_pct,roi_stddev,avg_position_size,total_volume,trades_per_day,avg_hold_hours,short_pct,last_trade,days_since_last',
    ...traders.map(w =>
      `${w.wallet},${w.total_trades},${w.wins},${w.losses},${w.win_rate_pct},${w.total_pnl},${w.median_roi_pct},${w.avg_roi_pct},${w.roi_stddev},${w.avg_position_size},${w.total_volume},${w.trades_per_day},${w.avg_hold_hours},${w.short_pct},${w.last_trade},${w.days_since_last}`
    )
  ].join('\n');

  const fs = require('fs');
  const csvPath = '/Users/scotty/Projects/Cascadian-app/active-consistent-roi-traders.csv';
  fs.writeFileSync(csvPath, csv);
  console.log(`✅ Exported ${traders.length} wallets to: ${csvPath}`);
}

findActiveConsistentROI().catch(e => {
