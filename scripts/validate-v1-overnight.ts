/**
 * V1/V1+ Overnight Validation - Optimized for reliability
 *
 * - Runs sequentially (not parallel) to avoid DB timeouts
 * - 5 minute timeout per query
 * - Only runs V1+ for high NR wallets (>100 conversions)
...
// Extended timeout client (5 min)
// Note: CLICKHOUSE_HOST already includes protocol
...
async function getNegRiskCount(wallet: string): Promise<number> {
  const query = `SELECT count() as cnt FROM vw_negrisk_conversions WHERE wallet = '${wallet}'`;
  const result = await clickhouse.query({ query, format: 'JSONEachRow' });
  const rows = (await result.json()) as any[];
  return Number(rows[0]?.cnt || 0);
}

async function getV1PnL(wallet: string): Promise<number> {
  const query = `
    WITH
      self_fills AS (
        SELECT transaction_hash FROM pm_trader_events_v3
      ...
      ctf AS (
        ...
        FROM pm_ctf_split_merge_expanded WHERE lower(wallet) = '${wallet}'
        GROUP BY condition_id, outcome_index
      ),
      combined AS (
        SELECT COALESCE(c.condition_id, f.condition_id) as cond,
          COALESCE(c.outcome_index, f.outcome_index) as outcome,
          COALESCE(c.clob_net, 0) + COALESCE(f.ctf_tokens, 0) as net,
          COALESCE(c.cash, 0) as cash
        FROM clob_pos c FULL OUTER JOIN ctf f ON c.condition_id = f.condition_id AND c.outcome_index = f.outcome_index
      ),
      resolved AS (
        SELECT p.*, toInt64OrNull(JSONExtractString(r.payout_numerators, p.outcome + 1)) = 1 as won
        FROM combined p JOIN pm_condition_resolutions r ON p.cond = r.condition_id AND r.is_deleted = 0
        WHERE r.payout_numerators IS NOT NULL AND r.payout_numerators != ''
      )
    SELECT round(sum(cash) + sumIf(net, net > 0 AND won) - sumIf(abs(net), net < 0 AND won), 2) as pnl FROM resolved
  `;
  const result = await clickhouse.query({ query, format: 'JSONEachRow' });
  const rows = (await result.json()) as any[];
...
async function getV1PlusPnL(wallet: string): Promise<number> {
  const query = `
    WITH
      self_fills AS (
        SELECT transaction_hash FROM pm_trader_events_v3
      ...
      clob_pos AS (
        ...
        JOIN pm_token_to_condition_map_v5 m ON t.token_id = m.token_id_dec
        WHERE lower(t.trader_wallet) = '${wallet}'
          AND m.condition_id != '' AND NOT (t.transaction_hash IN (SELECT transaction_hash FROM self_fills) AND t.role = 'maker')
        GROUP BY m.condition_id, m.outcome_index
      ),
      ctf AS (
        SELECT condition_id, outcome_index, sum(shares_delta) as ctf_tokens
      ...
      negrisk AS (
        ...
        JOIN pm_token_to_condition_map_v5 m ON toString(reinterpretAsUInt256(reverse(unhex(substring(v.token_id_hex, 3))))) = m.token_id_dec
        WHERE v.wallet = '${wallet}' AND m.condition_id != ''
        GROUP BY m.condition_id, m.outcome_index
      ),
      combined AS (
        SELECT COALESCE(c.condition_id, f.condition_id, n.condition_id) as cond,
          COALESCE(c.outcome_index, f.outcome_index, n.outcome_index) as outcome,
          COALESCE(c.clob_net, 0) + COALESCE(f.ctf_tokens, 0) + COALESCE(n.nr_tokens, 0) as net,
          COALESCE(c.cash, 0) as cash
        FROM clob_pos c
        FULL OUTER JOIN ctf f ON c.condition_id = f.condition_id AND c.outcome_index = f.outcome_index
        FULL OUTER JOIN negrisk n ON COALESCE(c.condition_id, f.condition_id) = n.condition_id
...
async function testWallet(w: { wallet: string; cohort: string }): Promise<Result> {
  ...
  try {
    // Get NR count first
    const nrCount = await getNegRiskCount(wallet);

    // Get V1
    const v1 = await getV1PnL(wallet);

    // Only get V1+ if NR > 100 (saves time for low-NR wallets)
    let v1plus: number | null = null;
    if (nrCount > 100) {
      v1plus = await getV1PlusPnL(wallet);
    }
    ...
    const v1pGap = v1plus !== null ? Math.abs(api - v1plus) : null;
    const bestGap = v1pGap !== null ? Math.min(v1Gap, v1pGap) : v1Gap;
    const bestEngine = v1pGap !== null && v1pGap < v1Gap ? 'V1+' : 'V1';
    const status = bestGap <= 10 ? 'PASS' : (bestGap <= 100 ? 'CLOSE' : 'FAIL');
    ...
    return {
      ...
      v1plus: v1plus !== null ? Math.round(v1plus * 100) / 100 : null,
      v1Gap: Math.round(v1Gap * 100) / 100,
      v1pGap: v1pGap !== null ? Math.round(v1pGap * 100) / 100 : null,
      bestGap: Math.round(bestGap * 100) / 100,
      bestEngine,
      status,
    };
  } catch (e: any) {
...
async function main() {
  ...
  const outputFile = `scripts/v1-overnight-${new Date().toISOString().replace(/:/g, '-').slice(0, 19)}.json`;

  console.log('=== V1/V1+ OVERNIGHT VALIDATION ===');
  ...
  for (const w of COHORT) {
    ...
    const elapsed = ((Date.now() - walletStart) / 1000).toFixed(1);
    console.log(`${result.status.padEnd(5)} (${elapsed}s) NR=${result.nrCount} Gap=$${result.bestGap}`);
  ...
  for (const cohort of cohorts) {
    const cr = valid.filter(r => r.cohort === cohort);
    const cp = cr.filter(r => r.status === 'PASS').length;
    console.log(`${cohort.padEnd(15)}: ${cp}/${cr.length} PASS`);
  }

  const totalTime = ((Date.now() - startTime) / 1000 / 60).toFixed(1);
  console.log(`\nCompleted in ${totalTime} minutes`);
