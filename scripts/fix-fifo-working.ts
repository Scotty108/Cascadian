/**
 * FIFO Fix - WORKING VERSION
 *
 * Fix: Create per-batch staging table (no WHERE IN needed)
 * Process entire staging table per batch
 */
...
async function processBatch(conditions: string[], batchNum: number, totalBatches: number): Promise<boolean> {
  try {
    ...
    await clickhouse.command({ query: 'DROP TABLE IF EXISTS tmp_batch_fills' });
    await clickhouse.command({ query: 'DROP TABLE IF EXISTS tmp_batch_conditions' });

    await clickhouse.command({
      query: `CREATE TABLE tmp_batch_conditions (condition_id String) ENGINE = Memory`
    });

    // Load conditions for this batch
    const values = conditions.map(id => `('${id}')`).join(',');
    await clickhouse.command({
      query: `INSERT INTO tmp_batch_conditions VALUES ${values}`
    });

    // Create staging table for this batch ONLY
    await clickhouse.command({
      query: `
        CREATE TABLE tmp_batch_fills
        (
    ...
    await clickhouse.command({
      query: `
        ...
        INNER JOIN tmp_batch_conditions m ON f.condition_id = m.condition_id
    ...
    // Cleanup
    await clickhouse.command({ query: 'DROP TABLE tmp_batch_conditions' });
    await clickhouse.command({ query: 'DROP TABLE tmp_batch_fills' });
...
async function main() {
  ...
  console.log('╔═══════════════════════════════════════════════╗');
  console.log('║         FIFO FIX - WORKING VERSION            ║');
  console.log('╚═══════════════════════════════════════════════╝');
