 ...
 * - FIFO v5 trade logic (is_closed flag for proper position tracking)
 ...
 * - First buy date >4 days ago
 * - Median ROI >0.15 (decimal, i.e., 15%)
 * - Median bet size >$10
 */
...
const LOOKBACK_DAYS = 30;       // Scoring window
const MIN_TRADES = 50;
const MIN_MARKETS = 8;
const ACTIVE_DAYS = 5;          // Must trade in last 5 days
const MIN_AGE_DAYS = 4;         // First buy >4 days ago
const MIN_MEDIAN_ROI = 0.15;    // Decimal (15%)
const MIN_MEDIAN_BET = 10;      // $10

// Walk-forward config
const WALK_FORWARD_WINDOWS = 3;
...
async function getQualifiedWallets(cutoffDate: Date): Promise<WalletStats[]> {
  ...
  // Step 1: Quick filter - get wallets with enough trades and activity
  console.log('    Phase 1: Finding active wallets with enough volume...');
  ...
  // Step 2: Detailed stats for candidates (in batches)
  console.log('    Phase 2: Computing detailed stats for candidates...');
...
async function getWalletTrades(wallet: string, startDate: Date, endDate: Date): Promise<Trade[]> {
  ...
  const query = `
    ...
    with_p95 AS (
      SELECT *, quantile(0.95)(trade_roi) OVER () as roi_p95
      FROM deduped
    )
    SELECT
      tx_hash,
...
function simulateCopier(trades: Trade[], startingBankroll: number): {
  ...
  // Calculate percent of bankroll per trade (equal scaling)
  // For simplicity: use fixed fraction like 1% per trade
...
async function walkForwardValidate(
  ...
  try {
    const trades = await getWalletTrades(wallet, forwardStart, forwardEnd);
    if (trades.length === 0) {
...
async function main() {
  ...
  console.log('COPYTRADING LEADERBOARD v12');
  ...
  try {
    ...
    for (let i = 0; i < top50.length; i++) {
      ...
      const wf = await walkForwardValidate(top50[i].wallet, DATA_CUTOFF);
    ...
    // Write to file
    const outputPath = './data/copytrading-leaderboard-v12.csv';
