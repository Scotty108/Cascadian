#!/usr/bin/env npx tsx
/**
 * SERVER-SIDE Wallet Metrics Rebuild - Uses Canonical P&L Pipeline
 *
 * KEY FIX: Uses trade_cashflows_v3 instead of trades_raw
 * APPROACH: Single server-side INSERT SELECT per window (no client-side batching)
 *
...
async function main() {
  ...
  console.log('\n' + '═'.repeat(100));
  console.log('WALLET METRICS REBUILD - SERVER-SIDE (Using Canonical P&L Pipeline)');
  ...
  try {
    ...
    // Step 3: Populate each window with server-side processing
    console.log('2️⃣  Populating windows (server-side processing)...\n');

    for (const window of windows) {
      console.log(`   Window: ${window.name} (≥ ${window.dateStart})...`);

      // Let ClickHouse do all the work server-side
      const insertSQL = `
        ...
        FROM (
          ...
          WHERE EXISTS (
            ...
            WHERE lower(tr.wallet) = lower(tcf.wallet)
              AND lower(replaceAll(tr.condition_id, '0x', '')) = tcf.condition_id_norm
              AND tr.block_time >= '${window.dateStart}'
