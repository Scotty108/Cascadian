 ...
 * Manual FIFO Refresh
 *
 * Runs the same logic as /api/cron/refresh-fifo-trades but as a standalone script
...
const BATCH_SIZE = 50;
const LOOKBACK_HOURS = 24; // Reduced from 48 to avoid memory limit

async function getRecentlyResolvedConditions(client: any): Promise<string[]> {
  // Use NOT EXISTS for better performance (ClickHouse optimizes this well)
...
async function main() {
  ...
  try {
    ...
    console.log('Finding recently resolved conditions...');
    ...
    let errors = 0;
    const numBatches = Math.ceil(conditions.length / BATCH_SIZE);
    ...
    for (let i = 0; i < conditions.length; i += BATCH_SIZE) {
      ...
      process.stdout.write(`Processing batch ${batchNum}/${numBatches} (${batch.length} conditions)... `);
