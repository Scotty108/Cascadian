/**
 * FIFO Fix - OPTIMIZED VERSION
 *
 * Based on original rebuild parameters that got 86% done:
 * - Batch size: 50 conditions (vs 200 that was timing out)
 * - Workers: 10 parallel (vs 5)
 * - Longer timeouts: 600 seconds (vs 300)
 */
...
const BATCH_SIZE = 50; // Smaller batches = faster staging population
...
async function processBatch(conditions: string[], batchNum: number, totalBatches: number): Promise<boolean> {
  ...
  try {
    ...
    // Populate staging - LONGER TIMEOUT
    await clickhouse.command({
      query: `
      ...
      `,
      clickhouse_settings: { max_execution_time: 600, max_threads: 4, max_memory_usage: 6000000000 }
    });
...
async function main() {
  ...
  console.log('╔═══════════════════════════════════════════════╗');
  console.log('║         FIFO FIX - OPTIMIZED VERSION          ║');
  console.log('╚═══════════════════════════════════════════════╝');
  ...
  for (let i = 0; i < conditionIds.length; i += BATCH_SIZE * PARALLEL_WORKERS) {
    ...
    const remaining = (totalBatches - completed) / rate;

    console.log(`Progress: ${completed}/${totalBatches} batches (${progress}%) | Elapsed: ${elapsed}m | ETA: ${remaining.toFixed(0)}m`);
  }
