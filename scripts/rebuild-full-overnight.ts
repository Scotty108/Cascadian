#!/usr/bin/env npx tsx
/**
 * PHASE 3: Full Overnight Backfill
 *
 * TRUE FIFO V5: Multiple rows per position (one per buy transaction)
 * GROUP BY (tx_hash, wallet, condition_id, outcome_index)
 *
 * Processes ALL 286M rows → ~192M deduplicated rows
 * Runtime: 2-4 hours
 */
...
async function rebuildFullOvernight() {
  const startTime = Date.now();
  console.log('🔨 PHASE 3: Full Overnight Backfill\n');
  console.log('⏰ Started at:', new Date().toLocaleString());
  console.log('📊 Target: 286M source rows → ~192M deduplicated rows\n');
  ...
  // Step 2: Insert in 200 batches
  const numBatches = 200;
  console.log(`2️⃣ Inserting FULL DATASET (${numBatches} batches)...\n`);
  ...
  for (let i = 0; i < numBatches; i++) {
    const batchNum = i + 1;
    console.log(`   Batch ${batchNum}/${numBatches}...`);
    ...
    try {
      ...
      if (batchNum % 10 === 0 || batchNum === numBatches) {
        ...
        const avgBatchTime = ((Date.now() - startTime) / 1000 / batchNum).toFixed(1);
        const etaSeconds = ((numBatches - batchNum) * parseFloat(avgBatchTime));
  ...
  // Step 6: SHORT positions check
  console.log('6️⃣ SHORT Positions Check...');
  ...
  console.log(`   Avg cost_usd (should be negative): $${shortData[0].avg_short_cost.toFixed(2)}\n`);

  const totalTime = ((Date.now() - startTime) / 1000 / 60).toFixed(1);
  const endTime = new Date().toLocaleString();

  console.log('═'.repeat(60));
  console.log(`✅ PHASE 3 COMPLETE in ${totalTime} minutes`);
  ...
  if (total === unique && batchErrors === 0) {
    console.log(`\n✅ PRODUCTION READY!`);
    console.log(`\n📋 The table pm_trade_fifo_roi_v3_mat_deduped is now complete and validated.`);
  } else if (batchErrors > 0) {
    console.log(`\n⚠️  WARNING: ${batchErrors} batch errors occurred. Review logs.`);
  } else {
