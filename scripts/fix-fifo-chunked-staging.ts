/**
 * FIFO Fix - Chunked Staging Table (Memory-Safe)
 *
 * Creates staging tables in chunks of 1000 conditions
 * Processes each chunk immediately, then moves to next
 *
 * ETA: 20-40 minutes
 */
...
const STAGING_CHUNK_SIZE = 1000; // Conditions per staging table
const BATCH_SIZE = 50; // Conditions per FIFO batch
...
async function processChunk(conditionChunk: string[], chunkNum: number, totalChunks: number) {
  ...
  // 1. Create temporary staging tables for this chunk
  await clickhouse.command({
    query: 'DROP TABLE IF EXISTS tmp_chunk_conditions'
  });

  await clickhouse.command({
    query: 'DROP TABLE IF EXISTS tmp_chunk_fills'
  });

  await clickhouse.command({
    query: `
      CREATE TABLE tmp_chunk_conditions (condition_id String)
      ENGINE = Memory
    `
  });

  // Load this chunk's conditions
  for (let i = 0; i < conditionChunk.length; i += 500) {
    const batch = conditionChunk.slice(i, i + 500);
    const values = batch.map(id => `('${id}')`).join(',');
    await clickhouse.command({
      query: `INSERT INTO tmp_chunk_conditions VALUES ${values}`
    });
  }
  ...
  // Populate staging table for this chunk
  ...
  await clickhouse.command({
    ...
    clickhouse_settings: {
      ...
      max_threads: 6, // Reduced to avoid memory spike
      max_memory_usage: 8000000000, // 8GB
    }
  });
  console.log(' ✓');

  // 2. Process this chunk in parallel batches
  ...
  for (let i = 0; i < conditionChunk.length; i += BATCH_SIZE * PARALLEL_WORKERS) {
    ...
    for (let w = 0; w < PARALLEL_WORKERS; w++) {
      ...
      workerPromises.push(
        processBatchFromStaging(conditionList).then(success => {
          if (success) {
...
async function processBatchFromStaging(conditionList: string): Promise<boolean> {
  try {
    // LONGS
    await clickhouse.command({
      query: `
      ...
      clickhouse_settings: {
        max_execution_time: 600,
        max_threads: 6,
        max_memory_usage: 4000000000, // 4GB per batch
...
async function main() {
  ...
  console.log('╔═══════════════════════════════════════════════╗');
  console.log('║  FIFO FIX - CHUNKED STAGING (MEMORY-SAFE)    ║');
  console.log('╚═══════════════════════════════════════════════╝');
