/**
 * Test V38 PnL engine on the known cohort
 */
...
async function main() {
  console.log('=== V38 PnL Cohort Test ===\n');
  ...
  for (const { wallet, name } of COHORT.passing) {
    ...
    const error = pmPnl !== null ? Math.abs(v38.total_pnl_mtm - pmPnl) : 0;
    const errorPct = pmPnl !== null && Math.abs(pmPnl) > 0 ? (error / Math.abs(pmPnl)) * 100 : 0;
    const status = errorPct < 10 ? '✅' : errorPct < 50 ? '⚠️' : '❌';

    results.push({ name, type: 'passing', v38Pnl: v38.total_pnl_mtm, pmPnl, error, errorPct, status });
    console.log(`  ${status} ${name}: V38=$${v38.total_pnl_mtm.toFixed(2)}, PM=${pmPnl !== null ? '$' + pmPnl.toFixed(2) : 'N/A'}, Err=${errorPct.toFixed(1)}%`);
  }
  ...
  for (const { wallet, name } of COHORT.failing) {
    ...
    results.push({ name, type: 'failing', v38Pnl: v38.total_pnl_mtm, pmPnl, error, errorPct, status });
    console.log(`  ${status} ${name}: V38=$${v38.total_pnl_mtm.toFixed(2)}, PM=${pmPnl !== null ? '$' + pmPnl.toFixed(2) : 'N/A'}, Err=${errorPct.toFixed(1)}%`);
  }
