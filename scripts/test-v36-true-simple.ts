/**
 * Test V36 on TRUE simple wallets - no PositionsConverted events
 */
...
async function main() {
  // Get wallets with NO PositionsConverted events
  const walletsResult = await clickhouse.query({
    query: `
      WITH no_negrisk_wallets AS (
        SELECT DISTINCT trader_wallet
        FROM pm_trader_events_v3 t
        WHERE NOT EXISTS (
          SELECT 1 FROM pm_ctf_events c
          WHERE lower(c.user_address) = lower(t.trader_wallet)
            AND c.event_type = 'PositionsConverted'
            AND c.is_deleted = 0
        )
      ),
      wallet_stats AS (
      ...
      SELECT w.trader_wallet, w.trade_count
      FROM no_negrisk_wallets n
      JOIN wallet_stats w ON n.trader_wallet = w.trader_wallet
      ORDER BY rand()
      LIMIT 20
    `,
    format: 'JSONEachRow',
  ...
  console.log('Testing V36 on 20 TRUE simple wallets (no PositionsConverted)...\n');
  ...
  for (const row of walletRows) {
    ...
    if (!wallet) {
      console.log('Skipping null wallet');
      continue;
    }
