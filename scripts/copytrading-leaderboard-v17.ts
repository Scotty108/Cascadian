#!/usr/bin/env npx tsx
/**
 * Copytrading Leaderboard v17 - Rank by Final Bankroll
 *
 * Change from v16:
 * - RANKS BY FINAL_BANKROLL instead of log_growth_per_day
 * - This directly answers: "Which wallets will make me the most money?"
 *
 * All v16 fixes retained:
 * 1. ROI floored at -100% (can't lose more than your investment)
 * 2. ROI capped at +200% (prevents overflow)
...
function generateCSV(results: SimulationResult[]): string {
  const headers = [
    'rank', 'wallet_address', 'polymarket_url', 'final_bankroll',
    'log_growth_per_day', 'simulated_copytrade_return_per_day', 'roi_per_day',
    'win_rate_percent', 'median_roi_percent', 'trades_per_day', 'ev_per_trade',
    'volatility', 'cvar', 'capital_velocity', 'hold_time_per_trade_minutes',
...
async function main() {
  ...
  console.log('COPYTRADING LEADERBOARD v17 - RANK BY FINAL BANKROLL');
  console.log('Dataset: ' + TABLE);
  console.log('='.repeat(80));
  console.log('Key change from v16:');
  console.log('  - RANKS BY FINAL_BANKROLL (not log_growth_per_day)');
  console.log('  - Directly answers: "Which wallets make me the most money?"');
  console.log('');
  console.log('All v16 fixes retained:');
  ...
  try {
    ...
    // ============================================
    // KEY CHANGE: RANK BY FINAL BANKROLL
    // ============================================
    console.log('Step 8: Ranking by FINAL BANKROLL (most money made)...');
    results.sort((a, b) => {
      if (b.final_bankroll !== a.final_bankroll) {
        return b.final_bankroll - a.final_bankroll;  // HIGHEST FIRST
      }
    ...
    if (results.length > 0) {
      console.log(`BEST WALLET (highest simulated return): ${results[0].wallet}`);
      console.log(`  Final Bankroll: $${results[0].final_bankroll.toFixed(2)} (from $${STARTING_BANKROLL})`);
      console.log(`  Log Growth/Day: ${results[0].log_growth_per_day.toFixed(8)}`);
      console.log(`  Trades Copied: ${results[0].trades_copied}`);
    ...
    const csvPath = resolve(process.cwd(), 'data', 'copytrading-leaderboard-v17.csv');
