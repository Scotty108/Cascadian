/**
 * Find Human Copy-Trading Candidates - V5
 * Filters:
 * 1. PnL > 0 (profitable)
 * 2. Active in last 10 days
 * 3. More than 10 trades
 * 4. Max 200 trades/day (not bots)
 * 5. NO CTF events (CLOB-only, no external transfers)
 */
...
async function main() {
  console.log('=== FINDING HUMAN COPY-TRADING CANDIDATES V5 ===\n');
  console.log('Filters: PnL > 0, 10+ trades, active 10 days, max 200/day, NO CTF events\n');
  ...
  // Step 3: Get wallets that have CTF events (to exclude)
  console.log('Step 3: Finding wallets with CTF events (to exclude)...');
  ...
  const ctfWallets = new Set((await ctfQuery.json() as any[]).map(w => w.wallet));
  console.log(`Wallets with CTF events: ${ctfWallets.size.toLocaleString()}\n`);
  ...
  console.log(`Filtered out - Has CTF events: ${ctfCount.toLocaleString()}`);

  console.log(`\n${'='.repeat(80)}`);
  console.log(`FINAL HUMAN CANDIDATES (CLOB-only): ${humanCandidates.length.toLocaleString()}`);
  console.log('='.repeat(80));

  if (humanCandidates.length === 0) {
    console.log('\nNo candidates found with all filters. This is very restrictive!');
    return;
  }
  ...
  console.log('\n=== 🎯 BEST COPY TRADING CANDIDATES ===');
  console.log('(70%+ WR, $1k+ PnL, <=30 trades/day, 20+ positions, CLOB-only)\n');
  ...
  fs.writeFileSync('/tmp/human-candidates-v5.json', JSON.stringify(humanCandidates, null, 2));
  fs.writeFileSync('/tmp/copy-targets-v5.json', JSON.stringify(copyTargets, null, 2));

  console.log(`\nFull list saved to /tmp/human-candidates-v5.json`);
  console.log(`Copy targets saved to /tmp/copy-targets-v5.json`);

  // Summary
  console.log(`\n=== SUMMARY ===`);
  console.log(`Total CLOB-only human candidates: ${humanCandidates.length.toLocaleString()}`);
  ...
  console.log(`Ideal copy targets: ${copyTargets.length}`);
}
