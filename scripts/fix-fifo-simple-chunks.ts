/**
 * FIFO Fix - SIMPLE CHUNKS
 *
 * Strategy:
 * 1. Create staging table for 1000 conditions at a time
 * 2. Process ENTIRE staging table in ONE batch (no sub-batching)
 * 3. This does the expensive INNER JOIN only 11 times
 * 4. NO WHERE condition_id IN filters needed - process entire staging table
 *
 * Expected: 11 chunks × 7 minutes = 77 minutes total
 */
...
async function processChunk(conditionChunk: string[], chunkNum: number, totalChunks: number) {
  ...
  try {
    ...
    process.stdout.write('  Loading conditions...');
    ...
    // Populate staging table - THIS IS THE EXPENSIVE OPERATION
    ...
    // Process LONGS - NO WHERE FILTER, process entire staging table
    ...
    console.log(`  Chunk complete in ${duration}m`);
    return true;
  } catch (err: any) {
...
async function main() {
  const startTime = Date.now();
  console.log('╔═══════════════════════════════════════════════╗');
  console.log('║         FIFO FIX - SIMPLE CHUNKS              ║');
  console.log('╚═══════════════════════════════════════════════╝');
  ...
  console.log(`Chunk size: ${CHUNK_SIZE} conditions`);
  ...
  for (let i = 0; i < conditionIds.length; i += CHUNK_SIZE) {
    ...
    const success = await processChunk(chunk, chunkNum, totalChunks);
    if (success) completedChunks++;
    else failedChunks++;
    ...
    console.log(`Overall: ${chunkNum}/${totalChunks} chunks (${progress}%) | Elapsed: ${elapsed}m | ETA: ${remaining}m\n`);
  }
  ...
  console.log('╚═══════════════════════════════════════════════╝');
  console.log(`Completed: ${completedChunks} chunks`);
