#!/usr/bin/env npx tsx
/**
 * Copytrading Leaderboard v14
 *
 * Using FIFO V5 logic from the reference guide:
 * - Realized PnL filter: (resolved_at IS NOT NULL OR is_closed = 1)
 * - Proper deduplication: GROUP BY tx_hash, wallet, condition_id, outcome_index
 *
 * Top 50 wallets ranked by Log Growth Per Day
 */
...
const MIN_MEDIAN_ROI = 0.15;
const MIN_MEDIAN_BET = 10;
const POSITION_FRACTION = 0.02;
...
function runUnitTests(): boolean {
  ...
  if (Math.abs(g1 - 0) > 0.0001) allPassed = false;

  const g2 = Math.log(101 / 100);
  if (Math.abs(g2 - 0.00995) > 0.0001) allPassed = false;

  const g3 = Math.log(99 / 100);
  if (Math.abs(g3 - (-0.01005)) > 0.0001) allPassed = false;

  // EV test
  const W = 0.6, Rw = 0.3, Rl = 0.2;
  const ev = W * Rw - (1 - W) * Rl;
  if (Math.abs(ev - 0.10) > 0.0001) allPassed = false;
...
async function getQualifiedWallets(cutoffDate: Date): Promise<WalletStats[]> {
  ...
  // FIFO V5: Include scalpers via (resolved_at IS NOT NULL OR is_closed = 1)
  const query = `
    WITH deduped AS (
      SELECT
      ...
      WHERE
        ...
        AND cost_usd >= 10
        AND (resolved_at IS NOT NULL OR is_closed = 1)  -- FIFO V5: Realized PnL only
...
async function getWalletTrades(wallet: string, startDate: Date, endDate: Date): Promise<Trade[]> {
  ...
  // FIFO V5: Include both resolved and closed (scalper) trades
  const query = `
    WITH deduped AS (
      SELECT
        tx_hash, wallet, condition_id, outcome_index,
...
async function main() {
  ...
  console.log('COPYTRADING LEADERBOARD v14 - FIFO V5 Logic');
  ...
  try {
    ...
    for (const stats of qualifiedWallets) {
      ...
      if (processed % 10 === 0 || processed === qualifiedWallets.length) {
        process.stdout.write(`\r  Processing ${processed}/${qualifiedWallets.length}... (${results.length} positive growth)`);
      }
    ...
    console.log('SUMMARY');
    console.log('='.repeat(80));
    console.log(`Total positive-growth wallets: ${results.length}`);
    if (results.length > 0) {
