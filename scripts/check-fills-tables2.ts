...
async function main() {
  // Sample vw_fills_normalized using correct column
  console.log('=== vw_fills_normalized - SAMPLE ===');
  const fillsSampleRes = await clickhouse.query({
    query: `SELECT * FROM vw_fills_normalized WHERE trader_wallet = '${THEO}' LIMIT 5`,
    format: 'JSONEachRow',
