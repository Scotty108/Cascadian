/**
 * V25 TDD Test Suite
 * Tests V25 (Deduped + AvgPrice) against all 15 test wallets
 */
import { config } from 'dotenv';
import { resolve } from 'path';
config({ path: resolve(process.cwd(), '.env.local') });

import { getWalletPnLV25 } from '../lib/pnl/pnlEngineV25';
...
async function runTests() {
  console.log('=== V25 DEDUPED + AVGPRICE ENGINE TEST SUITE ===\n');
  console.log('Name           | V25 Local    | API (truth)  | Diff      | Open | Status');
  console.log('---------------|--------------|--------------|-----------|------|-------');
  ...
  for (const w of TEST_WALLETS) {
    try {
      const [v25Result, apiPnl] = await Promise.all([
        getWalletPnLV25(w.wallet),
        fetchApiPnl(w.wallet)
      ]);

      const v25Total = v25Result.totalPnl;
      const apiTotal = apiPnl || 0;
      const absDiff = Math.abs(v25Total - apiTotal);
      const pctDiff = apiTotal !== 0 ? (absDiff / Math.abs(apiTotal)) * 100 : (v25Total === 0 ? 0 : 999);
      ...
      if (match) {
        ...
        failed++;
        failing.push(w.name + ': V25=' + v25Total.toFixed(0) + ' API=' + apiTotal.toFixed(0) + ' (' + pctDiff.toFixed(1) + '%)');
      }
      ...
      console.log(
        w.name.padEnd(14) + ' | $' +
        v25Total.toFixed(0).padStart(11) + ' | $' +
        apiTotal.toFixed(0).padStart(11) + ' | ' +
        diffStr.padStart(9) + ' | ' +
        v25Result.openPositionCount.toString().padStart(4) + ' | ' +
        status
      );
    } catch (error) {
      failed++;
      failing.push(w.name + ': ERROR - ' + (error as Error).message);
