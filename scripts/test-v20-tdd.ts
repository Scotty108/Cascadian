/**
 * V20 TDD Test Suite
 * Tests V20 (chronological position tracking) against all 15 test wallets
 */
import { config } from 'dotenv';
import { resolve } from 'path';
config({ path: resolve(process.cwd(), '.env.local') });

import { getWalletPnLV20 } from '../lib/pnl/pnlEngineV20';
...
async function runTests() {
  console.log('=== V20 CHRONOLOGICAL POSITION TRACKING TEST ===\n');
  console.log('Name           | V20 Local    | API (truth)  | Diff      | Phantom | Status');
  console.log('---------------|--------------|--------------|-----------|---------|-------');
  ...
  for (const w of TEST_WALLETS) {
    try {
      const [v20Result, apiPnl] = await Promise.all([getWalletPnLV20(w.wallet), fetchApiPnl(w.wallet)]);
      const v20Total = v20Result.totalPnl;
      const apiTotal = apiPnl || 0;
      const pctDiff = apiTotal !== 0 ? Math.abs((v20Total - apiTotal) / apiTotal) * 100 : (v20Total === 0 ? 0 : 999);
      const absDiff = Math.abs(v20Total - apiTotal);
      const match = pctDiff < 10 || absDiff < 5;
      if (match) passed++; else { failed++; failing.push(w.name + ': V20=' + v20Total.toFixed(0) + ' API=' + apiTotal.toFixed(0) + ' (' + pctDiff.toFixed(1) + '%)'); }
      const status = match ? 'PASS' : 'FAIL';
      const diffStr = pctDiff < 999 ? pctDiff.toFixed(1) + '%' : 'N/A';
      console.log(w.name.padEnd(14) + ' | $' + v20Total.toFixed(0).padStart(11) + ' | $' + apiTotal.toFixed(0).padStart(11) + ' | ' + diffStr.padStart(9) + ' | ' + v20Result.phantomSellsFiltered.toString().padStart(7) + ' | ' + status);
    } catch (error) {
