...
import { computeTwoLanePnl } from '@/lib/pnl/twoLanePnl';
...
async function main() {
  console.log('=== TWO-LANE P&L TEST ===\n');
  console.log('Lane 1 (Net Buyers): P&L = Sells + Redemptions - Buys');
  console.log('Lane 2 (Net Sellers): P&L = Sells + Redemptions - Buys - SplitCost\n');
  ...
  for (const w of TEST_WALLETS) {
    ...
    try {
      const result = await computeTwoLanePnl(w.address);

      console.log(`Lane: ${result.lane.toUpperCase()}`);
      console.log(`Engine P&L: $${result.realizedPnl.toFixed(2)}`);
      console.log(`  Tokens: bought=${result.tokensBought.toFixed(0)}, sold=${result.tokensSold.toFixed(0)}`);
