/**
 * Test Cash Parity Ledger on known wallets
 */

import { config } from 'dotenv';
config({ path: '.env.local' });
import { computeCashParityPnl } from '@/lib/pnl/cashParityLedger';
...
async function main() {
  console.log('=== CASH PARITY LEDGER TEST ===\n');
  console.log('This uses a clean sequential ledger with NO heuristic caps.\n');
  ...
  for (const w of TEST_WALLETS) {
    ...
    try {
      const result = await computeCashParityPnl(w.address);
      ...
      console.log(`  Split Cost: $${result.splitCost.toFixed(0)} (implicit: $${result.implicitSplits.toFixed(0)}, explicit: $${result.explicitSplits.toFixed(0)})`);
      console.log(`  Held Value: $${result.heldValue.toFixed(0)}`);
      console.log(`  Mapping: ${(result.mappingCoveragePct * 100).toFixed(1)}% (${result.mappedTokens}/${result.totalTokens} tokens)`);
