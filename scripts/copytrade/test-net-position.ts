/**
 * Test Net Position P&L
 */

import { config } from 'dotenv';
config({ path: '.env.local' });

import { computeNetPositionPnl } from '@/lib/pnl/netPositionPnl';
...
async function main() {
  console.log('=== NET POSITION P&L TEST ===\n');
  console.log('Split cost = max(sold - bought) per condition\n');
  ...
  for (const w of TEST_WALLETS) {
    ...
    try {
      const result = await computeNetPositionPnl(w.address);
