#!/usr/bin/env npx tsx
/**
 * Complete Wallet Metrics Rebuild - Full Coverage (CLI Version)
 *
 * Uses clickhouse-client for INSERT...SELECT to avoid HTTP header overflow
 *
 * Strategy:
 * 1. Get ALL unique wallets from trades_raw (lifetime scope)
 * 2. For each time window (30d, 90d, 180d, lifetime):
 *    - Insert row for EVERY wallet via clickhouse-client
...
// Get ClickHouse connection details from env
const CH_HOST = process.env.CLICKHOUSE_HOST || 'localhost';
const CH_PORT = process.env.CLICKHOUSE_PORT || '9000';
...
async function main() {
  ...
  console.log('\n' + '═'.repeat(100));
  console.log('WALLET METRICS COMPLETE REBUILD - FULL 923k×4 COVERAGE (CLI VERSION)');
  ...
  try {
    ...
    // Step 4: Populate each window with ALL wallets using clickhouse-client
    console.log('3️⃣  Populating all windows with complete wallet coverage (via CLI)...\n');

    for (const window of windows) {
      console.log(`   ${window.name} (block_time >= ${window.dateStart})...`);

      // Build INSERT...SELECT query for clickhouse-client
      const insertSQL = `
      ...
      // Execute via clickhouse-client CLI
      const startTime = Date.now();

      const passwordArg = CH_PASSWORD ? `--password="${CH_PASSWORD}"` : '';
      const cmd = `echo "${insertSQL.replace(/"/g, '\\"')}" | clickhouse-client --host="${CH_HOST}" --port="${CH_PORT}" --user="${CH_USER}" ${passwordArg}`;
      ...
      try {
        ...
        console.error(`     ❌ Failed to insert via CLI: ${error.message}`);
        throw error;
      }
    }
