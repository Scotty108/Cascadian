...
import { getWalletPnLV4 } from '../lib/pnl/pnlEngineV4';
...
async function main() {
  console.log('=== V4 Test: CTF-Aware Split Detection ===\n');
  console.log('# | V4 PnL       | UI PnL       | Diff   | Split | CLOB  | Match');
  console.log('--|--------------|--------------|--------|-------|-------|------');

  let passed = 0;
  for (let i = 0; i < wallets.length; i++) {
    const w = wallets[i];
    const r = await getWalletPnLV4(w.wallet);
    const diff = w.ui !== 0 ? ((r.total - w.ui) / Math.abs(w.ui)) * 100 : (r.total === 0 ? 0 : 999);
    const match = Math.abs(diff) < 10 ? '✅' : '❌';
    if (Math.abs(diff) < 10) passed++;

    console.log(`${(i+1).toString().padStart(2)} | $${r.total.toFixed(2).padStart(11)} | $${w.ui.toFixed(2).padStart(11)} | ${diff.toFixed(1).padStart(5)}% | ${r.splitBuys.toString().padStart(5)} | ${r.clobBuys.toString().padStart(5)} | ${match}`);
  }
