...
async function main() {
  ...
  console.log(`Using: erc20_transfers_staging (387M+ rows)\n`);

  try {
    // Get schema of staging table
    console.log("Step 1: Analyzing staging table schema...\n");
    ...
    console.log("Step 2: Fetching USDC transfers for wallet cluster...\n");
    ...
    console.log("\nAmount (USD)  Direction    From/To Address                    Timestamp             TxHash");
    ...
    for (let i = 0; i < Math.min(20, sorted.length); i++) {
      ...
      const txShort = t.tx.substring(0, 8) + "...";
      console.log(
        `${t.value.toLocaleString("en-US", { maximumFractionDigits: 2 }).padStart(12)}  ${direction}     ${partner.padEnd(36)} ${timestamp.padEnd(20)} ${txShort}`
      );
    }
