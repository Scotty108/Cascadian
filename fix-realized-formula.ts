...
async function main() {
  console.log("\n══════════════════════════════════════════════════════════════");
  console.log("FIX: CORRECT realized_pnl_by_market_final FORMULA");
  console.log("══════════════════════════════════════════════════════════════\n");

  console.log("ISSUE: Formula has inverted signs");
  console.log("Current: net_shares - cashflows → produces negative P&L");
  console.log("Correct: cashflows - net_shares → produces positive P&L\n");
  ...
  // Step 2: Recreate with CORRECTED formula
  console.log("\n📋 Step 2: Recreating realized_pnl_by_market_final with CORRECTED formula...\n");

  const createRealizedPnl = `
    CREATE VIEW default.realized_pnl_by_market_final AS
    WITH win AS (
      SELECT 
        condition_id_norm, 
    ...
    SELECT 
      ...
      round(sum(toFloat64(c.cashflow_usdc)) - sumIf(toFloat64(p.net_shares), p.outcome_idx = w.win_idx), 4) AS realized_pnl_usd
  ...
  if (result.success) {
    console.log(`  ✅ Created realized_pnl_by_market_final with CORRECTED formula\n`);
  } else {
  ...
  const createWalletRealizedPnl = `
    CREATE VIEW default.wallet_realized_pnl_final AS
    SELECT 
      wallet,
