...
async function main() {
  console.log("\n════════════════════════════════════════════════════════════════");
  console.log("REBUILDING P&L VIEWS WITH CORRECT FORMULA");
  console.log("Formula: realized_pnl = sum(cashflows) for resolved markets");
  console.log("════════════════════════════════════════════════════════════════\n");
  ...
  // Create realized_pnl_by_market_final (mirroring v2 logic)
  console.log("\nStep 2: Creating realized_pnl_by_market_final...\n");
  ...
  const createRealizedByMarket = `
    ...
    SELECT 
      ...
      round(sum(tcf.cashflow_usdc), 2) AS realized_pnl_usd,
      count() AS fill_count
