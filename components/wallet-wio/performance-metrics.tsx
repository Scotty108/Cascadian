...
export function PerformanceMetrics({
  ...
  const getMetricsForWindow = (window: TimeWindow) =>
    allMetrics.find(m => m.window_id === window);

  const allTimeMetrics = getMetricsForWindow('ALL');
    ...
    <Card className="p-6 border-border/50">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          subtext={metrics.brier_mean < 0.2 ? 'Excellent' : metrics.brier_mean < 0.25 ? 'Good' : 'Average'}
          icon={<Target className="h-4 w-4" />}
          tooltip="Prediction accuracy (0 = perfect, 0.25 = random). Lower is better."
          valueColor={metrics.brier_mean < 0.2 ? 'text-[#00E0AA]' : metrics.brier_mean < 0.25 ? 'text-amber-500' : 'text-muted-foreground'}
        />
