...
async function main() {
  console.log("\n════════════════════════════════════════════════════════════════════");
  console.log("FIX REALIZED PnL VIEW - CORRECT SETTLEMENT FORMULA");
  console.log("════════════════════════════════════════════════════════════════════\n");

  // The correct formula should be:
  // Realized PnL = Sum of winning shares * $1 - Sum of all cashflows spent on that outcome
  // = sumIf(net_shares, outcome_idx = win_idx) + sum(-cashflow_usdc)
  // The negative sign is because cashflow_usdc is already signed (negative for buys, positive for sells)
...
GROUP BY p.wallet, p.market_id, p.condition_id_norm, w.resolved_at`;
  ...
  if (await executeQuery("Recreate realized_pnl_by_market_final", fixView)) {
    // Test the new view
    ...
    const result = await ch.query({
      query: `
        ...
        FROM (
          SELECT 
            p.wallet AS p_wallet,
            realized_pnl_usd
          FROM realized_pnl_by_market_final
          WHERE p.wallet IN ('0xa4b366ad22fc0d06f1e934ff468e8922431a87b8', '0xeb6f0a13ea8c5a7a0514c25495adbe815c1025f0')
        )
        GROUP BY p_wallet
      `,
      format: 'JSON'
