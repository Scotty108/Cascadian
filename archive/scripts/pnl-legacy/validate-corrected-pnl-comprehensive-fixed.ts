...
/**
 * Comprehensive P&L Validation with All Three Fixes (FIXED VERSION)
 *
 ...
 *
 * Usage: npx tsx scripts/validate-corrected-pnl-comprehensive-fixed.ts [wallet_address]
 */
...
async function main() {
  ...
  console.log('COMPREHENSIVE P&L VALIDATION - ALL THREE FIXES (FIXED)');
  console.log('═'.repeat(80));
  ...
  console.log('  ✓ Bug #4: 100% token mapping coverage via ctf_token_map');
  console.log();

  // CORRECTED P&L CALCULATION WITH ALL FOUR FIXES
  // Using ctf_token_map to decode outcome indices
  const result = await clickhouse.query({
    query: `
      WITH positions_with_outcome AS (
        -- FIX #2/#4: Decode outcome index via JOIN with ctf_token_map
        SELECT
      ...
      cashflows_with_outcome AS (
        -- FIX #2/#4: Also decode outcome index for cashflows
        SELECT
          lower(cf.proxy_wallet) AS wallet,
          lower(replaceAll(cf.condition_id, '0x', '')) AS condition_id_norm,
          ctm.outcome_index AS outcome_idx,
  ...
  if (data.length === 0) {
    console.log('❌ ERROR: No resolved positions found for this wallet!');
    console.log();
    console.log('Possible causes:');
    console.log('  - gamma_resolved table is empty');
    console.log('  - ctf_token_map JOIN is failing');
  ...
  if (wins.length === 0) {
    ...
    wins.forEach((row: any, idx: number) => {
      ...
      const cidDisplay = row.condition_id_norm?.substring(0, 16) || '(unknown)';

      console.log(`${(idx + 1).toString().padStart(2)}. ${cidDisplay}... (outcome ${row.outcome_idx}, winner: ${row.winning_outcome})`);
  ...
  console.log('═'.repeat(80));
  console.log('[3] Outcome Index Distribution (Bug #2/#4 verification)');
  console.log('═'.repeat(80));
  console.log();
  ...
  if (outcome1Count === 0) {
    ...
    console.log('    Bug #2/#4 fix may not be working correctly.');
  } else {
  ...
  if (Math.abs(variance) < 2.0) {
    ...
    console.log();
    console.log('All FOUR bug fixes verified:');
    ...
    console.log('  ✅ 100% token mapping coverage (ctf_token_map complete)');
    console.log();
