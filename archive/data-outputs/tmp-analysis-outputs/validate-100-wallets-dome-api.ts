...
async function fetchDomePnL(wallet: string): Promise<number | null> {
  try {
    ...
    console.log(`    Calling Dome API: ${url.replace(DOME_API_KEY!, '[REDACTED]')}`)
    ...
    if (!data.pnl_over_time || data.pnl_over_time.length === 0) {
      console.log(`    ⚠️  No P&L data returned from Dome`)
      return null
    }

    // Get the most recent P&L value (last in array)
    const latestPnl = data.pnl_over_time[data.pnl_over_time.length - 1].pnl_to_date
    console.log(`    ✅ Dome P&L: $${latestPnl.toLocaleString()}`)

    return latestPnl

  } catch (error: any) {
    console.log(`    ⚠️  Error fetching from Dome: ${error.message}`)
    return null
  }
}
...
async function validate100Wallets() {
  ...
  try {
    ...
    console.log('DOME API VALIDATION: 100 RANDOM WALLETS')
    console.log('=' .repeat(80))
    console.log(`Started: ${new Date().toISOString()}`)
    console.log(`Dome API: ${DOME_API_BASE_URL}`)
    console.log('=' .repeat(80))
    console.log('')

    // Sample 100 random wallets with significant activity
    console.log('Step 1: Sampling 100 random wallets...')
    console.log('  Strategy: Top 100 by absolute P&L (most active traders)')
    console.log('')
    ...
    console.log('Step 2: Validating against Dome API...')
    console.log(`  Note: Free tier rate limits apply, this may take a while`)
    console.log('')
    ...
    for (let i = 0; i < wallets.length; i++) {
      ...
      console.log(`[${i + 1}/${wallets.length}] ${walletAddr}`)
      console.log(`  Our P&L: $${ourPnL.toLocaleString()} (${ourMarkets} markets)`)
      ...
      if (domePnL === null) {
        ...
        results.push({
          ...
          status: 'api_error',
          error: 'Failed to fetch from Dome API'
        })
        console.log('')
      ...
      console.log(`  Difference: $${diff.toLocaleString()} (${diffPct.toFixed(2)}%)`)
    ...
    console.log(`Total wallets sampled: ${wallets.length}`)
    ...
    if (successfulResults.length > 0) {
      ...
      console.log(`  ✅ Excellent (<1% error):   ${excellent} (${(excellent / successfulResults.length * 100).toFixed(1)}%)`)
      console.log(`  ✅ Good (1-2% error):       ${good} (${(good / successfulResults.length * 100).toFixed(1)}%)`)
      console.log(`  ⚠️  Acceptable (2-5% error): ${acceptable} (${(acceptable / successfulResults.length * 100).toFixed(1)}%)`)
      console.log(`  ❌ Needs Review (>5% error): ${needsReview} (${(needsReview / successfulResults.length * 100).toFixed(1)}%)`)
      console.log('')
    ...
    if (outliers.length > 0) {
      console.log('Wallets needing review (>5% error):')
      outliers.forEach(r => {
        console.log(`  ${r.wallet}`)
        console.log(`    Our P&L:   $${r.our_pnl.toLocaleString()}`)
        console.log(`    Dome P&L:  $${r.dome_pnl!.toLocaleString()}`)
    ...
    const csvPath = 'tmp/dome-validation-100.csv'
    const csvHeader = 'wallet,our_pnl,dome_pnl,difference,difference_pct,our_markets,status,error\n'
    ...
    if (successfulResults.length === 0) {
      console.log('❌ UNABLE TO VALIDATE: All Dome API requests failed')
      console.log('   Check API key, rate limits, or network connectivity')
    } else {
      const passRate = ((excellent + good) / successfulResults.length * 100).toFixed(1)

      if (excellent + good >= successfulResults.length * 0.95) {
        console.log(`✅ EXCELLENT: ${passRate}% of wallets within 2% error`)
        console.log('   P&L calculations are highly accurate')
        ...
        console.log('   P&L calculations are acceptable for production')
      } else {
        console.log(`⚠️  NEEDS WORK: Only ${passRate}% within 2% error`)
        console.log('   Investigate outliers before production deployment')
      }
    }
