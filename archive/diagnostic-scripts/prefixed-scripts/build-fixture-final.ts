 ...
 * BUILD FIXTURE - FINAL
 *
 * Filter ERC1155 transfers to our wallet only
 */
...
async function main() {
  console.log('═══════════════════════════════════════════════════════════');
  console.log('FIXTURE BUILDER (FINAL)');
  console.log(`Wallet: ${TARGET_WALLET}`);
  console.log('═══════════════════════════════════════════════════════════\n');

  // Get fills with token_ids
    ...
    query: `
      WITH fill_tokens AS (
        ...
        WHERE f.proxy_wallet = '${TARGET_WALLET}'
          AND (e.from_address = '${TARGET_WALLET}' OR e.to_address = '${TARGET_WALLET}')
      )
      SELECT
        f.tx_hash,
        f.timestamp,
        ...
        f.fee_rate_bps,
        ft.token_id,
        lpad(lower(hex(bitShiftRight(toUInt256(ft.token_id), 8))), 64, '0') as condition_id_norm,
        toUInt8(bitAnd(toUInt256(ft.token_id), 255)) as outcome_index
      FROM clob_fills f
      INNER JOIN fill_tokens ft
        ON f.tx_hash = ft.tx_hash
      WHERE f.proxy_wallet = '${TARGET_WALLET}'
