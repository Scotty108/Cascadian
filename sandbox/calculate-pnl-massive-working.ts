...
async function calculatePnLMassiveWorking() {
  ...
  for (const [key, pos] of positions.entries()) {
    if (pos.realized_pnl !== 0 || pos.total_sell_proceeds > 0) {
      market_count++;
      results.push({
        market: key.slice(-8),
...
calculatePnLMassiveWorking().catch(console.error);","file_path":"/Users/scotty/Projects/Cascadian-app/sandbox/calculate-pnl-massive-working.ts"}